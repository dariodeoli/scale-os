...
const teamPayload={week:'2026-09-14',scope:'team',automatic:[
 {user_id:'1',actor_name:'Ana López',actor_verified:true,counts:{video:2,reedicion:0,foto:1,produccion:0,entregable:0,untyped:1}},
 {user_id:'2',actor_name:'Bruno',actor_verified:true,counts:{video:0,reedicion:1,foto:0,produccion:2,entregable:0,untyped:0}},
]};
const ownPayload={week:'2026-09-14',scope:'own',automatic:[{user_id:'1',actor_name:'Ana López',actor_verified:true,counts:{video:2,reedicion:0,foto:1,produccion:0,entregable:0,untyped:1}}]};
...
async function main(){
 // AR-3 team scope: per-collaborator rows with finished counts by type, no hours.
 await mount('owner');
 ...
 assert.deepEqual(renderer.root.findAllByType('td').map(cell=>cell.children.join('')),['2','0','1','0','0','1','0','1','0','2','0','0'],'finished counts render per collaborator and type');
 ...
 assert.equal(renderer.root.findAllByType('td').length,6);
 renderer.unmount();
 ...
 renderer.unmount();
 console.log('PASS: weekly automatic section renders audit-derived type counts per collaborator, own/team scope, Monday normalization, empty state, retry and no hour fields');
}
