...
const findButton=(label:string,instance:any)=>instance.root.findAllByType('button').find(node=>text(node).includes(label)||String(node.props['aria-label']||'').includes(label));
async function main(){
 let renderer:any;
 ...
 assert(rendererB.root.findAllByType('input').some(node=>node.props.type==='url'),'el modo enlace muestra el campo de URL');
