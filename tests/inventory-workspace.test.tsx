...
async function run(){
 ...
 const viewControl=renderer.root.findByProps({role:'group','aria-label':'Vista de inventario'});let viewButtons=viewControl.findAllByType('button');assert.equal(viewButtons.length,3,'grid, list and pipeline are the equipment views');assert(viewButtons.every(node=>node.props['aria-label']&&node.props['aria-pressed']!==undefined),'view controls retain accessible labels and selected state');act(()=>viewButtons[1].props.onClick());assert.match(tree(),/inventory-equipment-list/,'list control keeps the dense list view available');viewButtons=viewControl.findAllByType('button');assert.equal(viewButtons[1].props['aria-pressed'],true);act(()=>viewButtons[0].props.onClick());
