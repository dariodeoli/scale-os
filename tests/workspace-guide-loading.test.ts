...
test('a superseded or invalidated load cannot overwrite data or guide evidence',async()=>{
 ...
 assert.deepEqual(h.state.guide,{scope:null,status:'unknown'});assert.equal(h.state.clients.length,1);
  ...
  const clearScoped=home.body!.statements.find(n=>ts.isFunctionDeclaration(n)&&n.name?.text==='clearScopedShellData')!;
  const setters:Record<string,unknown>={dataLoadSequence:h.sequence,userRef:{current:null}};
  const setterNames=(clearScoped.getText(ast)+'\n'+clearSession.getText(ast)).match(/\bset\w+(?=\()/g)!;
