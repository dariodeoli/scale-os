...
for(const width of [320,360,390,540,760]){
 assert.equal(declarations('.login-page',width)['grid-template-columns'],'minmax(0,1fr)');
 assert.equal(declarations('.login-page .login-card',width)['overflow-wrap'],'anywhere');
 assert.equal(declarations('.dialog-body fieldset',width)['min-width'],'0');
 for(const selector of ['.dialog-body select','.dialog-body textarea','.login-card input']){
  assert.equal(declarations(selector,width)['min-height'],'44px');assert.equal(declarations(selector,width)['font-size'],'16px');
 }
 assert.equal(declarations('.dialog-heading .icon-button',width).width,'44px');
 assert.equal(declarations('.dialog-footer .dialog-actions>button',width)['max-width'],'100%');
}
for(const width of [768,1024])assert.equal(declarations('.dialog-body select',width)['min-height'],undefined,'desktop form density unchanged');
...
console.log('PASS: shared mobile form CSS contracts at 320/360/390/540/760, unchanged desktop sizing, and 54 dropdown viewport positions; not visual browser QA');
