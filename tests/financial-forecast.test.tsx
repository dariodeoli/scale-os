...
let renderer:ReactTestRenderer;const rendered=()=>JSON.stringify(renderer.toJSON());
const byClass=(name:string)=>renderer.root.findAll(node=>typeof node.props?.className==='string'&&node.props.className.split(' ').includes(name)).length;async function respond(request:Request,data:unknown,status=200){await act(async()=>{request.resolve(new Response(JSON.stringify(data),{status}));});}
const buttonText=(button:{props:{children?:unknown}}):string=>{const value=button.props.children;return (Array.isArray(value)?value:(value===undefined||value===null?[]:[value])).filter((item):item is string=>typeof item==='string').join('');};
async function run(){
 ...
 act(()=>renderer.root.findAllByProps({type:'month'})[0].props.onChange({target:{value:'2026-10'}}));assert.match(requests.at(-1)!.url,/month=2026-10/);await respond(requests.at(-1)!,{...fixture,month:'2026-10',records:[],contracted_recurring:{month:'2026-10',records:[]},invoiced:{month:'2026-10',records:[]},collected_actual:{month:'2026-10',records:[]},commission_forecast:{month:'2026-10',records:[]},planned_expenses:{month:'2026-10',records:[]}});assert.match(rendered(),/Sin facturas emitidas/);act(()=>renderer.unmount());
 ...
 const hostClass=(name:string)=>renderer.root.findAll(node=>node.type==='span'&&typeof node.props?.className==='string'&&node.props.className.split(' ').includes(name));
  const maskedBases=hostClass('forecast-person-base');
 ...
 const maskedTotals=hostClass('forecast-person-total');
 ...
 assert.equal(byClass('forecast-person-actions'),0,'a masked salary offers no blind salary edit');
 ...
 const currencySelect=renderer.root.findAllByProps({id:'forecast-expense-currency'})[0];
 assert.equal(renderer.root.findAllByType('option').length,6,'the planned-expense currency select offers the six company currencies');
 act(()=>currencySelect.props.onChange('EUR'));
 assert.equal(renderer.root.findAllByProps({id:'forecast-expense-currency'})[0].props.value,'EUR','picking EUR in the selector is not silently reset to PYG');
 ...
 act(()=>renderer.unmount());
 // Vacíos con salida (ronda 14, #62): la previsión ofrece registrar la primera
 // factura o navegar a clientes/equipo sin duplicar modales.
 let invoiceClicks=0;const navigations:string[]=[];
 const emptyForecast:ForecastData={...fixture,records:[],contracted_recurring:{month:'2026-09',records:[]},invoiced:{month:'2026-09',records:[]},collected_actual:{month:'2026-09',records:[]},commission_forecast:{month:'2026-09',records:[]},planned_expenses:{month:'2026-09',records:[]},personnel:{month:'2026-09',included_headcount:'0',records:[]},contracted_clients:{month:'2026-09',records:[]}};
 await act(async()=>{renderer=create(<FinancialForecast role="finance" organizationId="one" navigate={label=>navigations.push(label)} onCreateInvoice={()=>{invoiceClicks+=1;}}/>);});await act(async()=>{});
 await respond(requests.at(-1)!,emptyForecast);
 assert.match(rendered(),/Sin facturas emitidas/);
 const invoiceCta=renderer.root.findAllByType('button').find(button=>buttonText(button).includes('Registrar factura'));
 assert(invoiceCta,'la previsión vacía ofrece registrar la primera factura');
 act(()=>invoiceCta!.props.onClick());assert.equal(invoiceClicks,1,'el CTA de factura sale por el shell, sin duplicar el modal');
 const teamCta=renderer.root.findAllByType('button').find(button=>buttonText(button).includes('Ver equipo'));
 assert(teamCta,'el vacío de personal lleva a Equipo');act(()=>teamCta!.props.onClick());
 const clientsCta=renderer.root.findAllByType('button').find(button=>buttonText(button).includes('Ver clientes'));
 assert(clientsCta,'el vacío de contratos lleva a Clientes');act(()=>clientsCta!.props.onClick());
 assert.deepEqual(navigations,['Equipo','Clientes']);
 act(()=>renderer.unmount());
 console.log('PASS financial forecast: role gating, canonical numeric/string response transport, top-level segregated aggregates, per-currency rendering, exact planned-expense POST, refresh, horizon selection with multi-month projection tables, contracted versus invoiced per client, masked per-person salaries (salary.view), the six company currencies with the salary kept in the record currency, and empty states with contextual exits (#62)');
}
void run();
