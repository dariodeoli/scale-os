...
async function run(){
 ...
 act(()=>{renderer=create(<ClientCommercialLifecycle id="1" role="sales"/>);});await respond(latest(),fixture());assert.match(rendered(),/Contenido mensual/);assert.match(rendered(),/Gs\./);act(()=>renderer.root.findByType('button').props.onClick());
