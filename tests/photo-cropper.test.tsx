...
const draws:unknown[][]=[];let closedBitmaps=0,fetches=0,hasContext=true;
Object.assign(globalThis,{
 ...
 document:{createElement:()=>({width:0,height:0,getContext:()=>hasContext?{drawImage(...args:unknown[]){draws.push(args);}}:null,toDataURL:(type:string,quality:number)=>{assert.equal(type,'image/webp');assert.equal(quality,.92);return encoded;}})},
});
async function main(){
 const area={x:200,y:0,width:800,height:800};
 assert.equal(await cropImage(source,area),encoded);
 ...
 assert.equal(closedBitmaps,4,'All decoded bitmaps close, including failed crops');
