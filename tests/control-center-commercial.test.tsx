...
async function run(){
 ...
 await mount('sales');assert.deepEqual(requests.map(request=>request.url),['/core-api/api/agency/control-center'],'sales never requests the financial dashboard');await respond(requests[0],salesCommercial);assert.match(text(),/No disponible para tu rol/);assert.doesNotMatch(text(),/No se pudo validar el resumen comercial/,'the canonical permission state is not treated as malformed data');assert.doesNotMatch(text(),/USD.*400/,'withheld expected billing is never rendered from an overbroad response');act(()=>renderer.unmount());
 ...
 console.log('PASS commercial control center: canonical route, snake_case response, currency-aware contracted billing totals, unavailable values and role boundaries');
}
void run();
