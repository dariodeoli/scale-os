...
async function run(){
 ...
 assert.equal(requests.length,0,'forbidden roles and invalid IDs never fetch');
  await act(async()=>{renderer=create(<ClientReporting id="1" role="finance"/>);});await load();assert.match(JSON.stringify(renderer.toJSON()),/Solo lectura/);assert.match(JSON.stringify(renderer.toJSON()),/Términos comerciales efectivos/);for(const control of [...renderer.root.findAllByType(SelectCustom),...renderer.root.findAllByType('input')])assert.equal(control.props.disabled,true);act(()=>renderer.unmount());
  await act(async()=>{renderer=create(<ClientReporting id="1" role="sales"/>);});await act(async()=>{});const salesStart=requests.length-1;assert.equal(requests[salesStart].url,'/core-api/api/agency/clients/1/reporting');assert.equal(requests.length,salesStart+1,'non-financial roles never fetch commercial-terms');await respond(requests[salesStart],reporting('1',{servicePlanId:null,relationshipStartedOn:null}));assert.equal(renderer.root.findAllByType(SelectCustom).length,2,'non-financial roles see only the reporting fields');const salesTree=JSON.stringify(renderer.toJSON());assert.ok(!salesTree.includes('Monto recurrente')&&!salesTree.includes('Términos comerciales efectivos'),'cuánto paga nunca se renderiza para gerencia o ventas');act(()=>renderer.root.findAllByType(SelectCustom)[0].props.onChange('company'));act(()=>{void saveButton().props.onClick();});const reportingOnly=requests.at(-1)!;assert.equal(reportingOnly.url,'/core-api/api/agency/clients/1/reporting');assert.deepEqual(json(reportingOnly),{expectedVersion:'9007199254740993',customerKind:'company'},'reporting PATCH keeps its versioned contract and never embeds commercial terms');await respond(reportingOnly,reporting('1',{customerKind:'company'}));assert.match(JSON.stringify(renderer.toJSON()),/Datos para reportes guardados/);act(()=>renderer.unmount());
  await act(async()=>{renderer=create(<ClientReporting id="1" role="owner"/>);});await load(reporting('1',{servicePlanId:null,relationshipStartedOn:null}),commercial(null));assert.match(JSON.stringify(renderer.toJSON()),/Todavía no hay términos comerciales efectivos/);assert.equal(renderer.root.findAllByType(SelectCustom).length,6,'editor is present even without saved terms and commission is optional');
  const selects=renderer.root.findAllByType(SelectCustom);const inputs=renderer.root.findAllByType('input');
  const setSelect=(index:number,value:string)=>act(()=>renderer.root.findAllByType(SelectCustom)[index].props.onChange(value));
  ...
  setSelect(2,'7');setByPlaceholder('1.000.000','2500000');act(()=>renderer.root.findAllByType('input').filter(input=>input.props.type==='date')[1].props.onChange({target:{value:'2024-02-01'}}));act(()=>renderer.root.findAllByType('input').filter(input=>input.props.type==='date')[2].props.onChange({target:{value:'2024-03-31'}}));setSelect(4,'false');setSelect(5,'percentage');setSelect(6,'9');setByPlaceholder('0-100','25');
