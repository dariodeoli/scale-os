...
async function run(){
 ...
 act(()=>selects[2].props.onChange({target:{value:'7'}}));act(()=>inputs.find(input=>input.props.placeholder==='Sin separadores')!.props.onChange({target:{value:'2500000'}}));act(()=>inputs.filter(input=>input.props.type==='date')[1].props.onChange({target:{value:'2024-02-01'}}));act(()=>selects[4].props.onChange({target:{value:'false'}}));act(()=>selects[5].props.onChange({target:{value:'9'}}));act(()=>inputs.filter(input=>input.props.placeholder==='Sin separadores')[1].props.onChange({target:{value:'25'}}));
