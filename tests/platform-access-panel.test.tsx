...
async function mount(role:string|null='admin',currentUserId='1'){await act(async()=>{renderer=create(<PlatformAccessPanel currentUserId={currentUserId} platformRole={role}/>);});await respond(lastRequest(),fixture());}
async function run(){
 await act(async()=>{renderer=create(<PlatformAccessPanel currentUserId="1" platformRole="admin"/>);});
 assert.equal(lastRequest().url,'/core-api/api/platform/users?limit=100');
 await respond(lastRequest(),{error:'Sin conexión'},500);
 ...
 assert.ok(!text().includes('Hacer admin global')&&!text().includes('Eliminar usuario')&&!text().includes('Quitar acceso'),'viewers see no mutation controls');
 ...
 await respond(lastRequest(),fixture());
 ...
 act(()=>input.props.onChange({target:{value:'member@scale.example'}}));
 assert.equal(buttons().find(node=>textOf(node).includes('Eliminar definitivamente'))!.props.disabled,false);
 act(()=>{void buttons().find(node=>textOf(node).includes('Eliminar definitivamente'))!.props.onClick();});
 ...
 act(()=>renderer.unmount());
 console.log('PASS platform access panel: role-aware read-only view, access grants/revokes, email-confirmed user deletion with agency cascade notice, and self-deletion');
}
void run();
