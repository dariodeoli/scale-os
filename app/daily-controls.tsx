...
export function ClientReviewControl({orderId}:{orderId:string}){
 ...
 return <><button className="secondary" onClick={()=>{setOpen(true);void perform(load);}}>Revisión del cliente</button>{open&&<Dialog title="Compartir pieza para aprobación" close={()=>setOpen(false)}><p className="form-note">Primero completá las aprobaciones internas y agregá el enlace del archivo. El enlace de revisión vence en siete días. No se envía automáticamente.</p><button className="primary" disabled={busy} onClick={()=>void perform(async()=>{const d=await api<{url:string}>(`/api/agency/work-orders/${orderId}/client-review`,{});setUrl(d.url);await load();})}>Crear enlace para esta versión</button>{url&&<p><a href={url} target="_blank" rel="noreferrer">{url}</a></p>}{error&&<p role="alert" className="error">{error}</p>}{rows.map(r=><article className="ops-card" key={r.id}><h3>{({pending:'Pendiente',approved:'Aprobado',changes:'Cambios solicitados',revoked:'Desactivado'} as Record<string,string>)[str(r,'status')]}</h3><p>Enlace creado por <ActorIdentity name={str(r,'actor_name')} photoUrl={str(r,'actor_photo_url')} verified={r.actor_verified===true} timestamp={str(r,'created_at')}/></p>{str(r,'reviewer_name')&&<p>Respuesta del cliente <ActorIdentity name={str(r,'reviewer_name')} timestamp={str(r,'responded_at')}/></p>}<p>{str(r,'feedback')}</p><small>Vence: {str(r,'expires_at').slice(0,10)}</small>{r.status==='pending'&&<button className="text-button" disabled={busy} onClick={()=>void perform(async()=>{await api(`/api/agency/work-orders/${orderId}/client-review/${r.id}/revoke`,{});setUrl('');await load();})}>Desactivar enlace</button>}</article>)}</Dialog>}</>;
}
export function ClientPortalDeliveryControl({orderId,title,assetUrl}:{orderId:string;title:string;assetUrl:string}){
 ...
 return <><button className="secondary" onClick={()=>{setOpen(true);void load().catch(e=>setError(errorText(e)));}}>Portal del cliente</button>{open&&<Dialog title="Entregable para el portal" close={()=>setOpen(false)}><p className="form-note">Sólo se publica esta pieza aprobada. El cliente ve este enlace y puede comentar, aprobar o pedir cambios; no accede al panel interno.</p>{delivery?.visible?<><p className="success">Publicado · versión {str(delivery,'version')}</p><button className="text-button" onClick={async()=>{try{await api(`/api/agency/work-orders/${orderId}/client-portal-delivery`,{visible:false},'PATCH');await load();}catch(e){setError(errorText(e));}}}>Quitar del portal</button></>:<Editor fields={[{key:'title',label:'Título'},{key:'summary',label:'Contexto para el cliente',type:'textarea',optional:true},{key:'assetName',label:'Nombre del enlace'},{key:'assetUrl',label:'Enlace HTTPS del archivo',type:'url'}]} defaults={{title,summary:'',assetName:'Abrir archivo',assetUrl}} save={async values=>{await api(`/api/agency/work-orders/${orderId}/client-portal-delivery`,values);await load();}}/>}{error&&<p className="error" role="alert">{error}</p>}</Dialog>}</>;
}
export function ClientPortalAccess({clientId}:{clientId:string}){
 const [open,setOpen]=useState(false),[email,setEmail]=useState(''),[url,setUrl]=useState(''),[error,setError]=useState('');
 async function invite(){try{setError('');const result=await api<{url:string}>(`/api/agency/clients/${clientId}/client-portal-invites`,{email});setUrl(result.url);setEmail('');}catch(e){setError(errorText(e));}}
 return <><button className="secondary" onClick={()=>setOpen(true)}>Acceso del cliente</button>{open&&<Dialog title="Invitar al portal del cliente" close={()=>setOpen(false)}><p className="form-note">Generá un enlace personal para el correo elegido. Copialo y envialo sólo a esa persona.</p><label>Correo del cliente<input type="email" value={email} onChange={event=>setEmail(event.target.value)} autoComplete="email"/></label><button className="primary" disabled={!email} onClick={()=>void invite()}>Generar invitación</button>{url&&<><p className="success">Invitación creada. Vence en siete días.</p><input aria-label="Enlace de invitación" value={url} readOnly onFocus={event=>event.currentTarget.select()}/><button className="text-button" onClick={()=>void navigator.clipboard?.writeText(url)}>Copiar enlace</button></>}{error&&<p className="error" role="alert">{error}</p>}</Dialog>}</>;
}
