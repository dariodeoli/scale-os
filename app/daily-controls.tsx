...
export function ClientPortalAccess({clientId}:{clientId:string}){
 const [open,setOpen]=useState(false),[email,setEmail]=useState(''),[url,setUrl]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false),[confirming,setConfirming]=useState(''),[grants,setGrants]=useState<Row[]>([]);
