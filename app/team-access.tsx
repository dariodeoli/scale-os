...
export function TeamAccess({member,email,role,currentEmail,refresh,ambiguous=false}:{member:TeamMember|null;email:string|null;role:string;currentEmail:string;refresh:()=>Promise<void>;ambiguous?:boolean}){
 ...
 return <div className="team-access"><p><strong>Acceso al panel: {state}</strong>{member&&<> · {teamRoleLabels[member.role]||member.role}</>}</p><small className="form-note">El permiso define qué puede hacer en Scale OS; el cargo laboral se administra por separado en su ficha.</small>
