...
export default function ClientPasswordRecovery(){
 ...
 return <main className="client-portal"><section className="client-portal-card narrow"><p className="portal-status">SCALE OS · PORTAL DEL CLIENTE</p><h1>{isReset?'Elegí una contraseña nueva':'Recuperar contraseña'}</h1><p className="portal-muted">{isReset?'Usá una clave de al menos 8 caracteres; no hacen falta mayúsculas ni símbolos.':'Te enviaremos un enlace seguro si ese correo tiene acceso al portal.'}</p><form onSubmit={submit}>{isReset?<><label>Correo<EmailField value={email} onChange={setEmail} required/></label><PasswordField label="Nueva contraseña" name="password" value={password} onChange={setPassword} autoComplete="new-password" required minLength={8}/><PasswordField label="Confirmar contraseña" name="confirm" value={confirmation} onChange={setConfirmation} autoComplete="new-password" required minLength={8}/></>:<label>Correo<EmailField value={email} onChange={setEmail} required/></label>}{error&&<p className="error" role="alert">{error}</p>}{message&&<p className="success" role="status">{message}</p>}<button disabled={busy}>{busy?(isReset?'Guardando…':'Enviando…'):(isReset?'Guardar contraseña':'Enviar enlace')}</button><a className="portal-recovery-link" href="/cliente/ingresar">Volver a ingresar</a></form></section></main>;
}
