...
export default function ClientLogin(){
 ...
 return <main className="client-portal"><section className="client-portal-card narrow"><p className="portal-status">SCALE OS · PORTAL DEL CLIENTE</p><h1>Ver entregables</h1><p className="portal-muted">Ingresá con el correo y contraseña que configuraste al aceptar la invitación.</p><form onSubmit={submit}><label>Correo<input type="email" value={email} onChange={event=>setEmail(event.target.value)} autoComplete="email" required/></label><label>Contraseña<input type="password" value={password} onChange={event=>setPassword(event.target.value)} autoComplete="current-password" minLength={12} required/></label>{error&&<p className="error" role="alert">{error}</p>}<button disabled={busy}>{busy?'Ingresando…':'Ingresar'}</button><a className="portal-recovery-link" href="/cliente/recuperar">¿Olvidaste tu contraseña?</a></form><div className="portal-alternate-login"><a className="portal-google-icon" href="/core-api/api/client-portal/auth/google/start" aria-label="Continuar con Google" title="Continuar con Google"><span aria-hidden="true">G</span><span className="sr-only">Continuar con Google</span></a></div></section></main>;
}
