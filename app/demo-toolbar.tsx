...
export function DemoToolbar({role}:{role:string}){
 ...
 return <div className="demo-tools relative flex shrink-0 items-center gap-1.5 text-[11px] max-md:ml-auto" aria-label="Controles de la demo">
  <button className="demo-badge inline-flex min-h-8 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-fono/10 px-2 text-[11px] font-semibold text-fono-light transition hover:bg-ink-700 max-md:min-h-9 max-md:text-[10px]" type="button" onClick={()=>setInfo(true)} aria-label="Información de la demo"><Info size={14}/>Demo</button>
  <button className="demo-client-view inline-flex min-h-8 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg px-2 text-fono-light transition hover:bg-ink-700 max-md:min-h-9" type="button" onClick={()=>setClientView(true)}>Así te ve tu cliente</button>
  <select aria-label="Probar permiso" title="Probar otro permiso" className="min-h-8 w-[134px] rounded-lg border border-ink-600 bg-ink-800 px-2 text-[11px] font-medium text-fore max-md:min-h-9 max-md:w-[110px]" value={role} disabled={busy} onChange={async e=>{
