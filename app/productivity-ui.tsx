...
export function WorkPlanner({orders,userId,role,projects,openOrder,refresh,navigate}:{orders:WorkItem[];userId:string;role:string;projects:{id:string;name:string;client_name:string}[];openOrder:(id:string)=>void;refresh:()=>Promise<void>;navigate:(label:string)=>void}){
 const [view,setView]=useState('Mi día'),[selected,setSelected]=useState<string[]>([]),[month,setMonth]=useState(localDay().slice(0,7)),[templatesOpen,setTemplatesOpen]=useState(false),[batch,setBatch]=useState(false);
 ...
 return <section className="panel work-planner"><div className="panel-heading"><h2>Trabajo diario</h2><div className="choice-list compact">{['Mi día','Calendario','Lista y lotes'].map(v=><button key={v} className={view===v?'choice active':'choice'} onClick={()=>setView(v)}>{v}</button>)}</div></div>
  ...
  {!visible.length&&<p className="empty-copy">{view==='Mi día'?'No tenés piezas pendientes asignadas. Podés ver el tablero general debajo.':'No hay piezas para esta vista.'}</p>}
