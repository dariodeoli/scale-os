 ...
 return <article data-grid-card="equipment" className="flex min-h-[200px] flex-col gap-3 rounded-xl border border-ink-600 bg-ink-800 p-4">
  <div className="flex items-start justify-between gap-3">
   <div className="flex min-w-0 items-start gap-2">
    {selectable?<label className="mt-0.5 inline-flex shrink-0 items-center" title="Seleccionar para operar en lote"><input type="checkbox" className="h-4 w-4 p-0 accent-fono" aria-label={`Seleccionar ${item.name}`} checked={selected} onChange={onSelect}/></label>:null}
  ...
  <div className="mt-auto grid gap-2 border-t border-ink-600 pt-2">
   <div className="flex flex-wrap items-center justify-between gap-2">
    ...
    {canManage?<IconAction icon="check" tone="ok" disabled={verifying} label={verifying?'Verificando…':`Marcar verificado: ${item.name}`} onClick={()=>onVerify(item)}/>:null}
   </div>
...
function EquipmentRow({item,selectable,selected,onSelect,canManage,verifying,onDetail,onVerify,onVerifyDetail,onEdit,onArchive}:{
 ...
 return <article data-list-row="equipment" className={`${EQUIPMENT_GRID} min-h-[48px] rounded-xl border border-ink-600/60 bg-ink-800/40 px-3 py-2`} data-status={item.status}>
  <span className="flex items-center">{selectable?<input type="checkbox" className="h-4 w-4 p-0 accent-fono" aria-label={`Seleccionar ${item.name}`} checked={selected} onChange={onSelect}/>:null}</span>
   ...
   <div data-board="locations" className="flex snap-x gap-3 overflow-x-auto pb-1" role="region" aria-label="Pipeline de ubicaciones">
    {visibleColumns.map(column=><section key={column.key} data-board-column className="flex w-72 shrink-0 snap-start flex-col gap-2 rounded-xl border border-ink-600 bg-ink-800/60 p-3" data-readonly={column.readOnly?'true':undefined}>
     <header className="flex items-center gap-2">
      ...
      {column.key==='sin-ubicacion'?<IconAction icon="close" label="Ocultar columna Sin ubicación" onClick={()=>setHideUnassigned(true)}/>:null}
     </header>
...
function PipelineCard({item,canManage,onDetail,onQuickVerify,verifyingId}:{item:InventoryItem;canManage:boolean;onDetail:(item:InventoryItem)=>void;onQuickVerify:(item:InventoryItem)=>void;verifyingId:string|null}){
 ...
 return <article ref={draggable.setNodeRef} style={style} data-board-card data-status={item.status} className={`grid gap-2 rounded-xl border border-ink-600 bg-ink-800 p-2.5 ${draggable.isDragging?'opacity-60':''}`}>
  ...
  <div className="flex items-center justify-between gap-2">
   <StateChip tone={statusTone(item.status)}>{equipmentStatusLabel(item.status)}</StateChip>
   {!disabled?<span className="flex items-center gap-1">
 ...
 return <div className="grid min-w-0 gap-4">
  <Card className="grid min-w-0 gap-4">
   ...
   <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
    <SegmentedField ariaLabel="Vistas de inventario" value={view} onChange={(value:string)=>setView(value as 'equipment'|'reservations')} options={[['equipment','Equipos','box'],['reservations','Calendario y reservas','calendar']]}/>
   ...
   {view!=='reservations'?<div className="grid gap-3">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
     ...
     <div className="flex flex-wrap items-center gap-2">
      ...
      <SegmentedField ariaLabel="Vista de inventario" value={equipmentView} onChange={(value:string)=>setEquipmentView(value as 'grid'|'list'|'pipeline')} options={[['grid','Cuadrícula','grid'],['list','Lista','list'],['pipeline','Ubicaciones','store']]}/>
  ...
  {view==='reservations'?<Card className="grid min-w-0 gap-4">
   ...
   <div data-list="reservations" className="min-w-0 overflow-x-auto">
    ...
    {reservations.map(row=><div key={row.id} data-list-row="reservations" className={`${RESERVATION_GRID} ${RESERVATION_COLS} min-h-[48px] rounded-xl border border-ink-600/60 bg-ink-800/40 px-3 py-2`} data-status={row.status}>
  ...
  {context?.can_manage?<Card className="grid min-w-0 gap-3">
...
function CategoryForm({category,done}:{category:Category|null;done:(message:string)=>void}){
 ...
 return <form className="grid gap-4" onSubmit={async event=>{event.preventDefault();if(busy)return;if(trimmed.length<2||trimmed.length>80){setError('El nombre debe tener entre 2 y 80 caracteres.');return;}setBusy(true);setError('');try{await api(`/api/agency/inventory-categories${category?`/${category.id}`:''}`,{name:trimmed,active,icon},category?'PATCH':'POST');done(category?'Categoría actualizada.':'Categoría creada.');}catch(error){setError(errorMessage(error));}finally{setBusy(false);}}}>
  ...
  {category?<label className="flex items-center gap-2 text-sm text-fore"><input type="checkbox" className="h-4 w-4 p-0 accent-fono" checked={active} onChange={(event:React.ChangeEvent<HTMLInputElement>)=>setActive(event.target.checked)}/><span>Disponible para nuevos equipos</span></label>:null}
  <fieldset className="grid gap-2"><legend className="text-[11px] font-medium uppercase tracking-wider text-mute">Ícono de categoría</legend><div className="flex flex-wrap gap-1" role="group" aria-label="Ícono de categoría"><button type="button" className={`grid h-9 w-9 place-items-center rounded-lg border ${icon===null?'border-fono bg-fono/15 text-fono-light':'border-ink-600 text-mute'}`} title="Sin ícono" aria-label="Sin ícono" aria-pressed={icon===null} onClick={()=>setIcon(null)}><X size={16}/></button>{Object.entries(categoryIconMap).map(([key,Icon])=><button key={key} type="button" className={`grid h-9 w-9 place-items-center rounded-lg border ${icon===key?'border-fono bg-fono/15 text-fono-light':'border-ink-600 text-mute'}`} title={key} aria-label={key} aria-pressed={icon===key} onClick={()=>setIcon(key)}><Icon size={16}/></button>)}</div></fieldset>
