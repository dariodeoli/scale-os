...
import {inventoryCode,printInventoryLabel} from './inventory-label';
...
function InventoryPanel(){
 ...
 function saved(message='Cambios guardados.'){setEditItem(null);setEditReservation(null);setEditCategory(null);setAction(null);setArchive(null);setSelectedItems([]);setReserveIds([]);setNotice(message);setRefresh(n=>n+1);}
 ...
 const visible=items.filter(item=>(!categoryFilter||String(item.category_id)===categoryFilter)&&`${inventoryCode(item.id)} ${item.name} ${item.serial_number||''} ${item.category_name||item.category} ${inventoryLocation(item)}`.toLowerCase().includes(search.toLowerCase()));
 return <div className="ops-stack inventory-workspace">
  <section className="panel"><div className="panel-heading"><div><h2>Inventario y reservas</h2><p className="form-note">Ubicación registrada y préstamo de equipos por producción.</p></div><div className="inline-actions">{context?.can_manage?<button className="secondary" onClick={()=>setEditItem('new')}>Agregar equipo</button>:null}{context?.can_reserve&&selectedItems.length?<button className="primary" onClick={()=>{setReserveIds(selectedItems);setEditReservation('new');}}>Reservar {selectedItems.length} seleccionado{selectedItems.length===1?'':'s'}</button>:context?.can_reserve?<button className="primary" onClick={()=>{setReserveIds([]);setEditReservation('new');}}>Reservar equipos</button>:null}</div></div>
   ...
   {loading?<p role="status">Cargando inventario…</p>:error?null:view==='equipment'?<>
    ...
    <div className="inventory-collection-toolbar"><p className="directory-summary">{visible.length} equipo{visible.length===1?'':'s'} visibles</p><ViewToggle label="Vista de inventario" value={equipmentView} onChange={setEquipmentView}/></div>
    <div className={`inventory-equipment-grid ${equipmentView==='list'?'inventory-equipment-list':''}`}>{visible.map(item=>{const selectable=context?.can_reserve&&!['maintenance','retired'].includes(item.status);const chosen=selectedItems.includes(String(item.id)),code=inventoryCode(item.id),location=inventoryLocation(item);return <article className="inventory-equipment" key={item.id}><div className="panel-heading"><div className="inventory-item-title">{selectable&&<label className="inventory-item-select"><input type="checkbox" aria-label={`Seleccionar ${item.name} para reservar`} checked={chosen} onChange={()=>setSelectedItems(current=>chosen?current.filter(id=>id!==String(item.id)):[...current,String(item.id)])}/></label>}<div><h3>{item.name}</h3><code className="inventory-code">{code}</code></div></div><span>{({available:'Disponible',in_use:'En uso',maintenance:'Mantenimiento',retired:'Dado de baja'} as Record<string,string>)[item.status]||item.status}</span></div><div className="inventory-item-meta"><small>{item.category_name||item.category||'Sin categoría'}{item.serial_number?` · Serie / IMEI: ${item.serial_number}`:''}</small><span className="inventory-location">{location}</span>{item.return_user_name?<span>Devuelve: {item.return_user_name}{item.expected_return_at?` · previsto ${dateTime(item.expected_return_at)}`:''}</span>:null}<small>{money(item.value,item.currency)}</small></div><div className="inline-actions inventory-item-actions"><button className="text-button" onClick={()=>printInventoryLabel({code,name:item.name,category:item.category_name||item.category||'Sin categoría',serial:item.serial_number,location})}>Etiqueta</button>{context?.can_manage?<><button className="text-button" onClick={()=>setEditItem(item)}>Editar equipo</button><button className="text-button" onClick={()=>setArchive(item)}>Archivar</button></>:null}</div></article>;})}</div>
  ...
  {editItem&&context?.can_manage?<Dialog title={editItem==='new'?'Nuevo equipo':editItem.name} close={()=>setEditItem(null)}><InventoryItemForm item={editItem==='new'?null:editItem} categories={categories} members={context.members} done={()=>refreshed('Equipo guardado.')}/></Dialog>:null}
  {editCategory&&context?.can_manage?<Dialog title={editCategory==='new'?'Nueva categoría':'Editar categoría'} close={()=>setEditCategory(null)}><Editor closeOnSave fields={[{key:'name',label:'Nombre de la categoría'},{key:'active',label:'Disponibilidad',choices:[{value:'true',label:'Activa'},{value:'false',label:'Archivada'}]}]} defaults={{name:editCategory==='new'?'':editCategory.name,active:String(editCategory==='new'||editCategory.active)}} save={async values=>{await api(`/api/agency/inventory-categories${editCategory==='new'?'':`/${editCategory.id}`}`,{name:values.name,active:values.active==='true'},editCategory==='new'?'POST':'PATCH');refreshed('Categoría guardada.');}}/></Dialog>:null}
...
function InventoryItemForm({item,categories,members,done}:{item:InventoryItem|null;categories:Category[];members:Person[];done:()=>void}){
 ...
 return <><p className="form-note">Un registro por unidad reservable. Al guardar se asigna un código único Scale OS, imprimible como etiqueta. Para un kit, indicá sus componentes en el nombre o las notas.</p>{item?<p className="form-note">Código de inventario: <code className="inventory-code">{inventoryCode(item.id)}</code></p>:null}<Editor closeOnSave fields={fields} defaults={defaults} save={async values=>{await api(`/api/agency/inventory${item?`/${item.id}`:''}`,values,item?'PATCH':'POST');done();}}/></>;
}
