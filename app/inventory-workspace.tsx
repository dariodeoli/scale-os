...
function InventoryPanel(){
 ...
 return <div className="ops-stack inventory-workspace">
  <section className="panel"><div className="inventory-toolbar"><div className="panel-heading inventory-title-block"><div><h2>Inventario y reservas</h2><p className="form-note">Ubicación registrada y préstamo de equipos por producción.</p></div></div><div className="inline-actions inventory-header-actions">{context?.can_manage?<button className="secondary" onClick={()=>setEditItem('new')}>Agregar equipo</button>:null}{context?.can_reserve&&!selectedItems.length?<button className="primary" onClick={()=>{setReserveIds([]);setEditReservation('new');}}>Reservar equipos</button>:null}</div>
   ...
   </>:<><label className="inventory-month">Mes del calendario<input type="month" value={month} min="1900-01" max="9998-12" onChange={e=>{if(/^\d{4}-(0[1-9]|1[0-2])$/.test(e.target.value))setMonth(e.target.value);}}/></label><p className="form-note">Horarios de Asunción. Se incluyen retiros pendientes de devolución aunque sean de otro mes.</p><InventoryCalendar month={month} reservations={reservations}/><div className="inventory-reservation-list">{reservations.map(row=><article key={row.id} className="inventory-reservation"><div className="inventory-reservation-identity"><h3>{row.title}</h3><small>{row.project_name}</small></div><span className="inventory-reservation-dates">{dateTime(row.starts_at)} → {dateTime(row.ends_at)}</span><span className="inventory-reservation-items">{row.items.map(i=>i.name).join(' · ')}</span><span className="inventory-reservation-people">Responsables: {row.responsible_members.map(p=>p.name).join(', ')} · Devuelve: {row.return_user_name}{row.status==='checked_out'?` · Custodio: ${row.custodian_name||'Sin registrar'}`:''}</span><span className="inventory-reservation-actors">{row.actor_name&&<span>Reservado por <ActorIdentity name={row.actor_name} photoUrl={row.actor_photo_url} verified={row.actor_verified===true}/></span>}{row.checkout_actor_name&&<span>Retiro: <ActorIdentity name={row.checkout_actor_name} photoUrl={row.checkout_actor_photo_url} verified={row.checkout_actor_verified===true}/></span>}{row.return_actor_name&&<span>Devolución: <ActorIdentity name={row.return_actor_name} photoUrl={row.return_actor_photo_url} verified={row.return_actor_verified===true}/></span>}</span><span className={`inventory-status inventory-status-${row.status}`}>{statusLabels[row.status]}</span>{row.status==='checked_out'&&new Date(row.ends_at)<new Date()?<p className="inventory-late">Devolución pendiente desde {dateTime(row.ends_at)}.</p>:null}{context&&(inventoryCanManageReservation(context,row)||row.status==='checked_out'&&inventoryCanReturn(context,row))?<div className="inline-actions inventory-reservation-actions">{row.status==='reserved'?<><button className="icon-button" type="button" title="Editar reserva" aria-label={`Editar reserva: ${row.title}`} onClick={()=>setEditReservation(row)}><Pencil size={16}/></button><button className="icon-button positive" type="button" title="Registrar retiro" aria-label={`Registrar retiro: ${row.title}`} onClick={()=>setAction({kind:'checkout',row})}><ArrowUpRight size={16}/></button><button className="icon-button warn" type="button" title="Cancelar reserva" aria-label={`Cancelar reserva: ${row.title}`} onClick={()=>setAction({kind:'cancel',row})}><X size={16}/></button></>:row.status==='checked_out'?<button className="icon-button positive" type="button" title="Registrar devolución" aria-label={`Registrar devolución: ${row.title}`} onClick={()=>setAction({kind:'return',row})}><RotateCcw size={16}/></button>:null}</div>:null}</article>)}</div>{!reservations.length?<p className="empty-copy">Sin reservas en este mes. Elegí equipos y fechas para planificar una producción.</p>:null}</>}
  </section>
