 ...
 return <div className="ops-stack inventory-workspace">
  <section className="panel"><div className="inventory-toolbar"><div className="panel-heading inventory-title-block"><div><h2>Inventario y reservas</h2><p className="form-note">Ubicación registrada y préstamo de equipos por producción.</p></div></div><div className="inline-actions inventory-header-actions">{context?.can_manage?<button className="secondary" onClick={()=>setEditItem('new')}>Agregar equipo</button>:null}{context?.can_reserve&&!selectedItems.length?<button className="primary" onClick={()=>{setReserveIds([]);setEditReservation('new');}}>Reservar equipos</button>:null}</div>
   ...
   {loading?<p role="status">Cargando inventario…</p>:error?null:view==='equipment'?equipmentView==='pipeline'?<InventoryPipeline items={items} locations={storageTemplates} canManage={Boolean(context?.can_manage)} onDetail={setDetail} onMoved={()=>refreshed('Ubicación actualizada.')} onQuickVerify={quickVerify} verifyingId={verifyingId}/>:<>
    ...
    <div className={`inventory-equipment-grid ${equipmentView==='list'?'inventory-equipment-list':''}`}>
   ...
   </>:<><label className="inventory-month">Mes del calendario<input type="month" value={month} min="1900-01" max="9998-12" onChange={e=>{if(/^\d{4}-(0[1-9]|1[0-2])$/.test(e.target.value))setMonth(e.target.value);}}/></label><p className="form-note">Horarios de Asunción. Se incluyen retiros pendientes de devolución aunque sean de otro mes.</p><InventoryCalendar month={month} reservations={reservations}/><div className="inventory-reservation-list">{reservations.length?<div className="inventory-reservation-head" aria-hidden="true"><span>Producción</span><span>Proyecto</span><span>Fechas</span><span>Equipos</span><span>Responsables</span><span>Devuelve</span><span>Acciones</span></div>:null}{reservations.map(row=><article key={row.id} className="inventory-reservation" data-status={row.status}><div className="inventory-reservation-title"><h3 title={row.title}>{row.title}</h3><span className={`inventory-status inventory-status-${row.status}`}>{statusLabels[row.status]}</span></div><span className="inventory-reservation-cell" title={row.project_name}>{row.project_name}</span><span className="inventory-reservation-cell" title={`${dateTime(row.starts_at)} → ${dateTime(row.ends_at)}`}>{dateTime(row.starts_at)} → {dateTime(row.ends_at)}</span><span className="inventory-reservation-cell" title={row.items.map(i=>i.name).join(' · ')}>{row.items.map(i=>i.name).join(' · ')}</span><span className="inventory-reservation-cell" title={row.responsible_members.map(p=>p.name).join(', ')}>{row.responsible_members.map(p=>p.name).join(', ')}</span><span className="inventory-reservation-cell" title={row.return_user_name||undefined}>{row.return_user_name||'—'}{row.status==='checked_out'?` · ${row.custodian_name||'Sin custodio'}`:''}</span>{context&&(inventoryCanManageReservation(context,row)||row.status==='checked_out'&&inventoryCanReturn(context,row))?<div className="inline-actions inventory-reservation-actions">{row.status==='reserved'?<><button type="button" className="icon-button" title="Editar reserva" aria-label={`Editar reserva: ${row.title}`} onClick={()=>setEditReservation(row)}><Pencil size={14}/></button><button type="button" className="icon-button positive" title="Registrar retiro" aria-label={`Registrar retiro: ${row.title}`} onClick={()=>setAction({kind:'checkout',row})}><Package size={14}/></button><button type="button" className="icon-button warn" title="Cancelar reserva" aria-label={`Cancelar reserva: ${row.title}`} onClick={()=>setAction({kind:'cancel',row})}><X size={14}/></button></>:row.status==='checked_out'?<button type="button" className="icon-button positive" title="Registrar devolución" aria-label={`Registrar devolución: ${row.title}`} onClick={()=>setAction({kind:'return',row})}><RefreshCw size={14}/></button>:null}</div>:null}{row.actor_name||row.checkout_actor_name||row.return_actor_name||row.status==='checked_out'&&new Date(row.ends_at)<new Date()?<p className="inventory-reservation-audit">{row.actor_name&&<span>Reservado por <ActorIdentity name={row.actor_name} photoUrl={row.actor_photo_url} verified={row.actor_verified===true}/></span>}{row.checkout_actor_name&&<span>Retiro por <ActorIdentity name={row.checkout_actor_name} photoUrl={row.checkout_actor_photo_url} verified={row.checkout_actor_verified===true}/></span>}{row.return_actor_name&&<span>Devolución por <ActorIdentity name={row.return_actor_name} photoUrl={row.return_actor_photo_url} verified={row.return_actor_verified===true}/></span>}{row.status==='checked_out'&&new Date(row.ends_at)<new Date()?<span className="inventory-late">Devolución pendiente desde {dateTime(row.ends_at)}</span>:null}</p>:null}</article>)}</div>{!reservations.length?<p className="empty-copy">Sin reservas en este mes. Elegí equipos y fechas para planificar una producción.</p>:null}</>}
  </section>
  ...
  {batchLocation?<Dialog title={`Mover ${batchLocation.length} equipo${batchLocation.length===1?'':'s'} de ubicación`} close={()=>setBatchLocation(null)}>{storageTemplates.some(template=>template.active)?<Editor columns fields={[{key:'location',label:'Ubicación',choices:storageTemplates.filter(template=>template.active).map(template=>({value:String(template.id),label:template.name}))},{key:'storage_row',label:'Fila / posición',optional:true}]} defaults={{location:String(storageTemplates.find(template=>template.active)?.id||''),storage_row:''}} label="Mover" save={async values=>{await batchMoveLocation(values.location,values.storage_row);}}/>:<Editor columns fields={[{key:'storage_shelf',label:'Ubicación',help:'Sin lugares configurados: escribí dónde se guardan.'},{key:'storage_row',label:'Fila / posición',optional:true}]} defaults={{storage_shelf:'',storage_row:''}} label="Mover" save={async values=>{await batchMoveLocation('',values.storage_row,values.storage_shelf);}}/>}</Dialog>:null}
   {detail?<Dialog title="Detalle y trazabilidad" close={()=>setDetail(null)}><InventoryDetail item={detail} members={context?.members||[]} canManage={Boolean(context?.can_manage)} onChanged={()=>refreshed('Mantenimiento actualizado.')}/></Dialog>:null}
...
export function InventoryItemForm({item,categories,members,storageTemplates,canManageStorage,createStorageTemplate,done}:{item:InventoryItem|null;categories:Category[];members:Person[];storageTemplates:StorageTemplate[];canManageStorage:boolean;createStorageTemplate:(name:string)=>Promise<StorageTemplate>;done:()=>void}){
 ...
 return <form className="inventory-form-grid inventory-item-form" onSubmit={submit}><p className="form-note inventory-wide">Un registro por unidad reservable. Al guardar se asigna un código único Scale OS, imprimible como etiqueta. Para un kit, indicá sus componentes en el nombre o las notas.</p>{item?<p className="form-note inventory-wide">Código de inventario: <code className="inventory-code">{itemCode(item)}</code></p>:null}
  ...
  <label>Valor de compra · Opcional<AmountInput value={values.purchase_value} currency={values.currency} onChange={value=>change('purchase_value',value)}/></label>
  <label>Fecha de compra · Opcional<input type="date" value={values.purchase_date} onChange={event=>change('purchase_date',event.target.value)}/></label>
  <label><SelectCustom label="Método de depreciación" choices={depreciationMethods} value={values.depreciation_method} onChange={value=>change('depreciation_method',value)}/></label>
  {values.depreciation_method==='linear'?<label>Vida útil (meses)<input type="number" min={1} max={600} step={1} required value={values.useful_life_months} onChange={event=>change('useful_life_months',event.target.value)}/></label>:null}
  <label>Valor residual<AmountInput value={values.residual_value} currency={values.currency} onChange={value=>change('residual_value',value)}/></label>
  {item?<p className="form-note inventory-wide">Valor actual {item.current_value==null?'sin calcular':money(item.current_value,values.currency)} · Depreciación acumulada {item.accumulated_depreciation==null?'sin calcular':money(item.accumulated_depreciation,values.currency)}. Se recalcula con los datos guardados; los cambios se aplican al guardar.</p>:null}
...
function InventoryMaintenanceForm({item,record,members,done}:{item:InventoryItem;record:InventoryMaintenance|null;members:Person[];done:()=>void}){
 ...
 return <form className="inventory-form-grid" onSubmit={submit}><p className="form-note inventory-wide">Mantenimiento de <code className="inventory-code">{itemCode(item)}</code>. El costo se registra en su moneda, sin conversiones.</p><label>Fecha del mantenimiento<input type="date" required value={values.maintenance_date} onChange={event=>change('maintenance_date',event.target.value)}/></label><label>Tipo de mantenimiento<input value={values.kind} required minLength={2} maxLength={80} placeholder="Preventivo, reparación, limpieza…" onChange={event=>change('kind',event.target.value)}/></label><label className="inventory-wide">Descripción · Opcional<textarea value={values.description||''} maxLength={2000} onChange={event=>change('description',event.target.value)}/></label><label>Costo<AmountInput value={values.cost} currency={values.currency} onChange={value=>change('cost',value)}/></label><label><SelectCustom label="Moneda" choices={currencyChoices} value={values.currency} onChange={value=>change('currency',value)}/></label><label><SelectCustom label="Responsable" choices={[{value:'',label:'Sin responsable'},...members.map(member=>({value:String(member.id),label:member.name}))]} value={values.responsible_user_id} onChange={value=>change('responsible_user_id',value)}/></label>{error?<p className="error inventory-wide" role="alert">{error}</p>:null}<SaveActions pending={busy}><button className="primary" disabled={busy}>{busy?'Guardando…':'Guardar mantenimiento'}</button></SaveActions></form>;
}
