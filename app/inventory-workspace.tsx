...
function PipelineColumn({column,canManage,onDetail,onQuickVerify,verifyingId}:{column:PipelineColumn;canManage:boolean;onDetail:(item:InventoryItem)=>void;onQuickVerify:(item:InventoryItem)=>void;verifyingId:string|null}){
 ...
 return <section ref={droppable.setNodeRef} className={`inventory-pipeline-column${droppable.isOver?' drop-over':''}${column.readOnly?' is-readonly':''}`}>
  <header className="inventory-pipeline-header"><span className="inventory-pipeline-column-dot" aria-hidden="true"/><h3>{column.title}</h3><span>{column.rows.length}</span></header>
...
function InventoryPanel(){
 ...
 const [month,setMonth]=useState(()=>inventoryLocalTime(new Date()).slice(0,7)),[view,setView]=useState<'equipment'|'pipeline'|'reservations'>('equipment'),[equipmentView,setEquipmentView]=useState<'grid'|'list'>('grid'),[selectedItems,setSelectedItems]=useState<string[]>([]),[reserveIds,setReserveIds]=useState<string[]>([]),[search,setSearch]=useState(''),[categoryFilter,setCategoryFilter]=useState('');
 ...
 return <div className="ops-stack inventory-workspace">
  <section className="panel"><div className="inventory-toolbar"><div className="panel-heading inventory-title-block"><div><h2>Inventario y reservas</h2><p className="form-note">Ubicación registrada y préstamo de equipos por producción.</p></div></div><div className="inline-actions inventory-header-actions">{context?.can_manage?<button className="secondary" onClick={()=>setEditItem('new')}>Agregar equipo</button>:null}{context?.can_reserve&&selectedItems.length?<button className="primary" onClick={()=>{setReserveIds(selectedItems);setEditReservation('new');}}>Reservar {selectedItems.length} seleccionado{selectedItems.length===1?'':'s'}</button>:context?.can_reserve?<button className="primary" onClick={()=>{setReserveIds([]);setEditReservation('new');}}>Reservar equipos</button>:null}</div>
   <div className="inventory-toolbar-meta"><div className="inline-actions inventory-tabs" role="group" aria-label="Vistas de inventario"><button className={view==='equipment'?'secondary':'text-button'} aria-pressed={view==='equipment'} onClick={()=>setView('equipment')}>Equipos</button><button className={view==='pipeline'?'secondary':'text-button'} aria-pressed={view==='pipeline'} onClick={()=>setView('pipeline')}>Pipeline</button><button className={view==='reservations'?'secondary':'text-button'} aria-pressed={view==='reservations'} onClick={()=>setView('reservations')}>Calendario y reservas</button></div><p className="form-note inventory-refresh-note" role="status">Sincroniza cada 30 s mientras esta pestaña esté visible.{lastUpdated?` Actualizado ${lastUpdated.toLocaleTimeString('es-PY',{hour:'2-digit',minute:'2-digit'})}`:''}</p></div>
   ...
   {loading?<p role="status">Cargando inventario…</p>:error?null:view==='pipeline'?<InventoryPipeline items={items} locations={storageTemplates} canManage={Boolean(context?.can_manage)} onDetail={setDetail} onMoved={()=>refreshed('Ubicación actualizada.')} onQuickVerify={quickVerify} verifyingId={verifyingId}/>:view==='equipment'?<>
  ...
  {editStorageTemplate&&context?.can_manage?<Dialog title={editStorageTemplate==='new'?'Nueva ubicación':'Editar ubicación'} close={()=>setEditStorageTemplate(null)}><StorageTemplateForm template={editStorageTemplate==='new'?null:editStorageTemplate} done={message=>saved(message)}/></Dialog>:null}
