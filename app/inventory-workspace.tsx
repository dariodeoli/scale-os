 ...
 return <article className="inventory-equipment" data-status={item.status}>
  <div className="panel-heading inventory-card-head">
   <div className="inventory-item-title">
    ...
    <div><h3>{item.name}</h3><code className="inventory-code">{code}</code></div>
   </div>
  ...
  <dl className="inventory-item-facts">
   <div><dt>Categoría</dt><dd><CategoryIcon name={item.category_icon}/>{item.category_name||item.category||'Sin categoría'}</dd></div>
   <div><dt>Ubicación</dt><dd title={location}>{location}</dd></div>
   <div><dt>Serie / IMEI</dt><dd title={item.serial_number||undefined}>{item.serial_number?<SerialTexto value={item.serial_number}/>:'Sin registrar'}</dd></div>
   <div><dt>Valor</dt><dd className="list-amount">{money(item.value,item.currency)}</dd></div>
  </dl>
