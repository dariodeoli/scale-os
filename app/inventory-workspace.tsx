...
export function InventoryItemForm({item,categories,members,storageTemplates,canManageStorage,createStorageTemplate,done}:{item:InventoryItem|null;categories:Category[];members:Person[];storageTemplates:StorageTemplate[];canManageStorage:boolean;createStorageTemplate:(name:string)=>Promise<StorageTemplate>;done:()=>void}){
 ...
 return <form className="inventory-form-grid inventory-item-form" onSubmit={submit}><p className="form-note inventory-wide">Un registro por unidad reservable. Al guardar se asigna un código único Scale OS, imprimible como etiqueta. Para un kit, indicá sus componentes en el nombre o las notas.</p>{item?<p className="form-note inventory-wide">Código de inventario: <code className="inventory-code">{itemCode(item)}</code></p>:null}
  ...
  <label>Categoría<select value={values.category_id} onChange={event=>change('category_id',event.target.value)}>{categories.filter(c=>c.active||String(c.id)===String(item?.category_id)).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
  ...
  <fieldset className="inventory-wide inventory-storage-field"><legend>Ubicación de guardado</legend><label>Ubicación<select value={templateId} onChange={event=>{const id=event.target.value;setTemplateId(id);const template=availableTemplates.find(candidate=>candidate.id===id);if(template)change('storage_shelf',template.name);}}><option value="">Ubicación personalizada</option>{availableTemplates.map(template=><option key={template.id} value={template.id}>{template.name}{template.active?'':' · archivada'}</option>)}</select></label>{!templateId?<label>Ubicación personalizada<input value={values.storage_shelf} onChange={event=>change('storage_shelf',event.target.value)} maxLength={100} placeholder="Estante, depósito o lugar"/></label>:<p className="form-note">Se guarda como {availableTemplates.find(template=>template.id===templateId)?.name||values.storage_shelf}.</p>}{canManageStorage?<div className="inventory-create-place"><label>Crear lugar<input value={newPlace} onChange={event=>setNewPlace(event.target.value)} maxLength={100} placeholder="Ej.: Depósito · Rack A"/></label><button type="button" className="text-button" disabled={!newPlace.trim()||creatingPlace} onClick={()=>void createPlace()}>{creatingPlace?'Creando…':'Crear lugar'}</button></div>:null}<label>Fila / posición<input value={values.storage_row} onChange={event=>change('storage_row',event.target.value)} maxLength={80}/></label></fieldset>
  <label>Valor del equipo<AmountInput value={values.value} currency={values.currency} onChange={value=>change('value',value)}/></label>
  <label>Moneda<select value={values.currency} onChange={event=>change('currency',event.target.value)}>{currencyChoices.map(choice=><option key={choice.value} value={choice.value}>{choice.label}</option>)}</select></label>
  <label>Estado<select value={values.status} onChange={event=>change('status',event.target.value)}>{(item?.status==='in_use'?[{value:'in_use',label:'En uso (registrar devolución)'}]:itemStatuses).map(choice=><option key={choice.value} value={choice.value}>{choice.label}</option>)}</select></label>
  <label>Custodio registrado<select value={values.custodian_user_id} onChange={event=>change('custodian_user_id',event.target.value)}><option value="">Sin custodio</option>{members.map(member=><option key={member.id} value={member.id}>{member.name}</option>)}</select></label>
...
function InventoryVerificationForm({item,done}:{item:InventoryItem;done:()=>void}){
 ...
 return <form className="inventory-form-grid" onSubmit={submit}><p className="form-note inventory-wide">Código: <code className="inventory-code">{itemCode(item)}</code>. El control queda fechado, asociado a tu usuario y no cambia reservas existentes.</p><label>Resultado<select value={result} onChange={event=>setResult(event.target.value as typeof result)}><option value="confirmed">Coincide con el registro</option><option value="difference">Hay una diferencia</option><option value="missing">No encontrado</option></select></label><label>Observación<textarea value={note} maxLength={2000} onChange={event=>setNote(event.target.value)} placeholder="Ej.: revisión mensual"/></label>{result!=='confirmed'?<label className="inventory-wide">Diferencias encontradas<textarea value={differences} required maxLength={2000} onChange={event=>setDifferences(event.target.value)} placeholder="Qué no coincide con el registro"/></label>:null}<label className="inventory-wide inventory-check"><input type="checkbox" checked={adjust} disabled={item.location_type==='checked_out'} onChange={event=>setAdjust(event.target.checked)}/><span>Ajustar estado o ubicación registrada{item.location_type==='checked_out'?' (no disponible mientras esté retirado)':''}</span></label>{adjust?<><label>Estado real<select value={status} onChange={event=>setStatus(event.target.value)}>{itemStatuses.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>Ubicación real<input value={shelf} maxLength={100} required onChange={event=>setShelf(event.target.value)}/></label><label>Fila / posición<input value={row} maxLength={80} onChange={event=>setRow(event.target.value)}/></label></>:null}{error?<p className="error inventory-wide" role="alert">{error}</p>:null}<SaveActions pending={busy}><button className="primary" disabled={busy}>{busy?'Registrando…':'Registrar verificación'}</button></SaveActions></form>;
}
