...
function EnlargedPhoto({photo,name,close}:{photo:string;name:string;close:()=>void}){
 ...
 return createPortal(<div className="photo-overlay" onClick={event=>{if(event.target===event.currentTarget)close();}}><section className="photo-dialog" ref={panel} role="dialog" aria-modal="true" aria-labelledby={title}>
  <div className="panel-heading"><h2 id={title}>Foto de {name}</h2><button type="button" className="icon-button" title="Cerrar foto ampliada" onClick={close} aria-label="Cerrar foto ampliada"><X size={20}/></button></div>
  ...
  <div className="photo-zoom-controls"><button type="button" className="secondary" disabled={zoom<=100} onClick={()=>setZoom(Math.max(100,zoom-50))} aria-label="Reducir foto"><ZoomOut size={18}/></button><label>Zoom <output>{zoom}%</output><input type="range" min="100" max="300" step="50" value={zoom} onChange={event=>setZoom(Number(event.target.value))} aria-label="Nivel de zoom de la foto"/></label><button type="button" className="secondary" disabled={zoom>=300} onClick={()=>setZoom(Math.min(300,zoom+50))} aria-label="Ampliar foto"><ZoomIn size={18}/></button><button type="button" className="text-button" onClick={()=>setZoom(100)}><Maximize size={14}/>Ajustar</button></div>
