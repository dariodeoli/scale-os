    ...
    {groupedRows(data.capabilities).map(group=><div key={group.name} className="contents">
     ...
     {group.rows.map(row=>{const manual=overrideCount(row);return <ListRow key={row.id} template={MATRIX_TEMPLATE}>
      ...
      <div className="flex min-w-0 items-center gap-x-4 whitespace-nowrap">
       {data.roles.map(roleId=>{const checked=effective(row,roleId);return <label key={roleId} className="relative flex min-h-11 items-center gap-2 whitespace-nowrap md:min-h-0 after:absolute after:inset-x-0 after:-inset-y-2.5 after:content-[''] max-md:after:content-none">
