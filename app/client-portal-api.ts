...
export async function portalApi<T>(path:string,body?:unknown,method='POST'):Promise<T>{
 const response=await fetch(clientPortalApiUrl(path),{method:body===undefined?'GET':method,credentials:'include',cache:'no-store',signal:AbortSignal.timeout(15000),headers:body===undefined?undefined:{'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)});
