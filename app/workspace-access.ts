...
export const visibleModule=(label:string,role:string)=>{
 ...
 if(['Pipeline','Mora'].includes(label))return ['owner','admin','management','finance','sales'].includes(role);
 if(['Planes','Presupuestos'].includes(label))return ['owner','admin','management','finance','sales','production'].includes(role);
 if(label==='Estudio')return ['owner','admin','management','production','finance','editor','viewer','sales'].includes(role);
