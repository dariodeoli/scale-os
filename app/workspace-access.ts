...
export const visibleModule=(label:string,role:string)=>{
 ...
 if(label==='Pipeline')return ['owner','admin','management','finance','sales','collaborator'].includes(role);
 if(label==='Mora')return ['owner','admin','management','finance','sales'].includes(role);
 if(['Planes','Presupuestos'].includes(label))return ['owner','admin','management','finance','sales','production','collaborator'].includes(role);
 if(label==='Estudio')return ['owner','admin','management','production','finance','editor','viewer','sales','collaborator'].includes(role);
