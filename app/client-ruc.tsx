...
export function ClientRuc({refresh,embedded=false,onCreated}:{refresh:()=>Promise<void>;embedded?:boolean;onCreated?:()=>void}){
 ...
 if(embedded)return <section className="client-ruc-inline" aria-label="Completar cliente desde RUC"><div><strong>Completar desde RUC</strong><small>Opcional · verificá razón social antes de crear.</small></div>{body}</section>;
 return <><button type="button" className="secondary" onClick={()=>setOpen(true)}>Agregar cliente por RUC</button>{open&&<Dialog title="Agregar cliente por RUC" close={close}>{body}</Dialog>}</>;
}
