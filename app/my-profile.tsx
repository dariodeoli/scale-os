...
export function MyProfile({profile,close,refresh}:{profile:Profile;close:()=>void;refresh:()=>Promise<void>}){
 ...
 return <Dialog title="Mi perfil" close={close}><div className="my-profile-content"><p className="form-note">Tu nombre y foto en esta empresa.</p>
  <Editor fields={[{key:'full_name',label:'Nombre completo'}]} defaults={{full_name:name}} save={async v=>{await api('/api/agency/productivity/profile',v,'PATCH');setName(v.full_name);await refresh();close();}}/>
