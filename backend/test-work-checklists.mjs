...
for(const name of ['20260908_treasury_ledger','20260908_people_commissions_comments','20260908_operations_complete',
 ...
 '20260910_productivity','20260910_work_checklists','20260910_profile_identity','20260910_demo_sessions','20260910_notifications','20260913_ruc_collaboration','20260914_production_traceability'])await pg.exec(await fs.readFile(`migrations/${name}.sql`,'utf8'));
...
await query("insert into agency_user_profiles(organization_id,user_id,full_name) values($1,$2,'Dueño')",[org,uid]);
...
let stamps=(await query('select completed_at,completed_by_user_id from agency_work_checklist_items where id=$1',[item])).rows[0];
assert.ok(stamps.completed_at,'checking stamps completed_at');
assert.equal(stamps.completed_by_user_id,uid,'checking attributes the transition actor');
assert.ok(result.items[0].completed_at,'snapshot exposes the stamp');
assert.equal(result.items[0].completed_by_name,'Dueño','snapshot resolves the completion actor identity');
...
stamps=(await query('select completed_at,completed_by_user_id from agency_work_checklist_items where id=$1',[item])).rows[0];
assert.ok(stamps.completed_at,'text-only edits preserve the completion stamp');
result=await call(`/items/${item}`,'PATCH',{completed:false,expected_version:'3'});assert.equal(result.completed,0);
stamps=(await query('select completed_at,completed_by_user_id from agency_work_checklist_items where id=$1',[item])).rows[0];
assert.equal(stamps.completed_at,null,'unchecking clears completed_at');
assert.equal(stamps.completed_by_user_id,null,'unchecking clears the completion actor');
assert.equal(result.items[0].completed_by_name,null,'unchecked items drop the actor identity');
result=await call(`/items/${item}`,'PATCH',{completed:true,expected_version:'4'});assert.equal(result.completed,1);
assert.equal((await call(`/items/${item}`,'PATCH',{text:'Guion aprobado',expected_version:'5'})).version,'5','no-op does not increment');
assert.equal((await call(`/items/${item}`,'PATCH',{completed:'false',expected_version:'5'})).status,400);
assert.equal((await call(`/items/${item}`,'PATCH',{expected_version:'5'})).status,400);
...
assert.equal((await call('/items','POST',{text:'Rollback',expected_version:'5'})).status,500);
assert.equal((await call()).version,'5');
await pg.exec('drop trigger reject_checklist_test on agency_work_checklist_items; drop function reject_checklist_test()');
result=await call(`/items/${item}`,'DELETE',{expected_version:'5'});assert.equal(result.version,'6');assert.equal(result.total,0);assert.equal(result.completed,0);
assert.equal((await call(`/items/${item}`,'DELETE',{expected_version:'6'})).status,404);
...
assert.equal((await call('/items','POST',{text:'101',expected_version:'6'})).status,400);
