...
assert.deepEqual(ids(card.effective_assignees),[String(direct),String(owner)],'Primary first and duplicate primary is collapsed');
assert.equal(card.effective_assignees[0].full_name,'Direct Person');assert.equal(card.effective_assignees[0].photo_url,'https://example.invalid/1.png');
assert.equal(card.effective_assignees[0].is_primary,true);assert.equal(Object.hasOwn(card.effective_assignees[0],'source'),false,'El origen va a nivel de fila');
assert.equal(Object.hasOwn(card,'assignees'),false,'El payload no duplica el detalle directo');assert.equal(Object.hasOwn(card,'project_assignees'),false,'El payload no duplica el detalle de proyecto');
assert.equal(Object.hasOwn(card,'assignee_email'),false);assert.equal(Object.hasOwn(card,'created_at'),false);assert.equal(Object.hasOwn(card,'organization_id'),false);assert.equal(Object.hasOwn(card,'assignee_version'),false);
card=r.data.workOrders.find(o=>o.id===inherited);
assert.equal(card.assignee_source,'project');assert.equal(Object.hasOwn(card,'assignees'),false,'El payload no duplica el detalle directo');
assert.deepEqual(ids(card.effective_assignees),[String(projectPerson),String(owner)]);
...
r=await read('work-orders/'+order);assert.equal(r.status,200);assert.equal(r.data.record.effective_assignees[0].photo_url,'https://example.invalid/1.png');
r=await read('productivity/orders/'+order);assert.equal(r.status,200);assert.equal(r.data.order.effective_assignees[0].full_name,'Direct Person');
...
assert.equal(r.status,200);assert.equal(r.data.workOrder.effective_assignees[0].full_name,'Direct Person');
rejectEnrichment=true;
...
r=await read('work-orders');card=r.data.workOrders.find(o=>o.id===order);assert.equal(card.effective_assignees[0].full_name,'Changed name');assert.equal(card.effective_assignees[0].photo_url,null);
...
assert.deepEqual(ids(card.effective_assignees),[String(owner)]);assert.deepEqual(card.assigned_user_ids,[String(owner)],'Inherited people never become stored direct IDs');
