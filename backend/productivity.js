...
export async function productivity({req,res,url,db,session,body,send}){
 ...
 try{
  ...
  if(kind==='history'&&req.method==='GET'){
   ...
   else if(req.method==='PATCH'&&key){const old=await owned(c,'agency_internal_tasks',key,org),b={...old,...await body(req)},person=await assignee(c,optId(b.assigned_user_id),org),title=text(b.title,160);if(title.length<2)fail('Ingresá el título');result={record:(await c.query('update agency_internal_tasks set title=$1,description=$2,status=$3,due_date=$4,assigned_user_id=$5,updated_at=now() where id=$6 returning *',[title,text(b.description,4000),option(b.status,['pending','in_progress','done']),date(b.due_date),person,key])).rows[0]};}
   else fail('Método no permitido',405);
