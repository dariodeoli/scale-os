...
// Lote de presupuestos (#59): archivar/restaurar con el mismo contrato que los demás lotes.
const budgetBatchA=(await query("insert into agency_budgets(organization_id,client_id,number,title,currency,subtotal,total,share_enabled) values($1,$2,'Q-BATCH-A','Lote A','USD',100,110,true) returning id",[org,client])).rows[0].id;
const budgetBatchB=(await query("insert into agency_budgets(organization_id,client_id,number,title,currency,subtotal,total,share_enabled) values($1,$2,'Q-BATCH-B','Lote B','USD',100,110,true) returning id",[org,client])).rows[0].id;
assert.equal((await call('/api/agency/budgets/batch','POST',{ids:Array.from({length:51},(_,index)=>String(index+1)),archived:true})).status,400,'máximo 50 por lote');
assert.deepEqual(await call('/api/agency/budgets/batch','POST',{ids:[budgetBatchA,budgetBatchB],archived:true}),{status:200,updated:2,archived:true});
assert.equal((await query("select count(*)::int as n from agency_archived_records where organization_id=$1 and kind='budgets' and record_id=any($2::bigint[])",[org,[budgetBatchA,budgetBatchB]])).rows[0].n,2,'quedan en la papelera');
assert.equal((await query('select count(*)::int as n from agency_budgets where id=$1 and share_enabled',[budgetBatchA])).rows[0].n,0,'al retirarlos se corta el enlace público');
assert.equal((await call('/api/agency/budgets/batch','POST',{ids:[budgetBatchA,'999999'],archived:false})).status,404,'todo id debe pertenecer a la empresa');
assert.equal((await call('/api/agency/budgets/batch','POST',{ids:[budgetBatchA],archived:true},{...user,role:'viewer'})).status,403,'viewer no opera el lote');
assert.deepEqual(await call('/api/agency/budgets/batch','POST',{ids:[budgetBatchA,budgetBatchB],archived:false}),{status:200,updated:2,archived:false});
assert.equal((await query("select count(*)::int as n from agency_archived_records where organization_id=$1 and kind='budgets' and record_id=any($2::bigint[])",[org,[budgetBatchA,budgetBatchB]])).rows[0].n,0,'se restauran por lote');
...
// #58: `?project_id=` acota por proyecto, es paginable y los conteos lo respetan.
const visibleProjectOrders=async projectId=>(await query(`select count(*)::int as total from agency_work_orders o join agency_projects p on p.id=o.project_id join agency_clients c on c.id=p.client_id where o.organization_id=$1 and o.project_id=$2 and ${visibleOrdersWhere}`,[org,projectId])).rows[0].total;
const projectOrders=await call(`/api/agency/work-orders?project_id=${project}`);
assert.equal(projectOrders.status,200);
assert.equal(projectOrders.workOrders.length,await visibleProjectOrders(project));
assert.equal(projectOrders.workOrders.every(row=>String(row.project_id)===String(project)),true,'el filtro devuelve solo ese proyecto');
const projectPage=await call(`/api/agency/work-orders?project_id=${project}&limit=1`);
assert.equal(projectPage.workOrders.length,1);
assert.equal(projectPage.page.hasMore,await visibleProjectOrders(project)>1,'la paginación compone con el filtro');
const projectCounts=await call(`/api/agency/work-orders?project_id=${project}&counts=1&limit=1&fields=id`);
assert.equal(Object.values(projectCounts.stage_counts).reduce((total,value)=>total+value,0),await visibleProjectOrders(project),'los conteos respetan el filtro de proyecto');
for(const invalid of ['abc','0','-1','1.5'])assert.equal((await call(`/api/agency/work-orders?project_id=${invalid}`)).status,400,`project_id inválido: ${invalid}`);
assert.equal((await call(`/api/agency/work-orders?project_id=${project}`,'GET',{}, {...user,role:'viewer'})).status,200,'el viewer puede filtrar por proyecto');
