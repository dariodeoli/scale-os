...
const visibleVersion=body=>body.replace(/<!--[\s\S]*?-->/g,'').includes(`v${expected}`);
await probe('Scale OS público',`${appOrigin}/status`,visibleVersion);
await probe('Registro público',`${appOrigin}/registro`,visibleVersion);
