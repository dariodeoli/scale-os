...
function fixtureSection(fixture) {
  ...
  const body = fixture.kind === 'workspace'
    ? `<main class="shell control-shell">${shellAside(fixture.section)}<section class="content flex min-h-dvh max-w-[1600px] min-w-0 flex-col min-[761px]:!w-[calc(100%-192px)]">
       <div class="workspace-topbar sticky top-0 z-20 flex min-h-14 items-center justify-between gap-3 border-b border-ink-600 bg-ink-800/95 px-4 py-2 md:px-6 lg:px-8 xl:px-12 max-md:z-30 max-md:grid max-md:grid-cols-1 max-md:gap-2" role="toolbar" aria-label="Controles del espacio de trabajo"><div class="topbar-primary flex min-w-0 items-center gap-3"><div class="topbar-identity flex min-w-0 items-center gap-2"></div><div class="topbar-workspace-context flex min-w-0 items-center gap-3"><div class="topbar-company min-w-0 [&_.company-name]:truncate [&_.workspace]:!m-0 [&_.workspace]:min-w-0 [&_.workspace]:overflow-hidden">Empresa de prueba</div></div></div><div class="topbar-utilities flex min-w-0 flex-wrap items-center justify-end gap-2 max-md:contents"><div class="topbar-status flex items-center gap-2 max-md:col-span-full max-md:row-start-2"></div><div class="topbar-utility-actions flex min-w-0 items-center gap-2 [&>*]:min-h-10 [&>*]:min-w-10 max-md:[&>*]:min-h-11 max-md:[&>*]:min-w-11"><span class="icon-button" aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg></span></div></div></div>
       <div class="flex min-w-0 flex-1 flex-col px-4 pb-8 pt-5 md:px-6 lg:px-8 xl:px-12">${fixture.body}</div>
       </section></main>`
    : fixture.body;
