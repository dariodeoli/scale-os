...
<section class="grid gap-4" aria-label="Finanzas">
 ${v2KpiStrip([
   ...
   v2Kpi('POR COBRAR', '<span class="flex flex-wrap items-baseline gap-2"><span>Gs. 987.654.321</span><span>USD 12.500</span></span>', 'Sobre las 24 facturas cargadas; “Ver todas las facturas” completa el total'),
...
<section class="${v2Card}" aria-labelledby="finance-transfers-title">
 <div class="flex flex-wrap items-center justify-between gap-2">
  <div class="min-w-0 flex-1"><h3 id="finance-transfers-title" class="text-[17px] font-semibold tracking-tight text-fore">Transferencias</h3><p class="mt-1 text-xs text-mute">Movimientos entre cuentas con su tipo de cambio real.</p></div>
 ...
 ${v2Grid({label: 'Transferencias entre cuentas', template: 'grid-cols-[minmax(20rem,1.6fr)_7rem_minmax(9rem,1.1fr)_minmax(9rem,1fr)_10rem]', columns: ['Transferencia', 'Fecha', 'Recibió', 'Referencia', 'Monto'], minWidth: 'min-w-[64rem]', rows: [
  v2Row('grid-cols-[minmax(20rem,1.6fr)_7rem_minmax(9rem,1.1fr)_minmax(9rem,1fr)_10rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Banco Regional — Operativa → Tarjeta corporativa USD">Banco Regional — Operativa → Tarjeta corporativa USD</b><small class="block truncate text-[11px] text-mute" title="Compra de dólares para campaña internacional">Compra de dólares para campaña internacional</small></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="17-sept-26 · 10:00">17-sept</span></div><div class="min-w-0 flex-1">${actorCompact('FD', 'Fredd D.')}</div><div class="min-w-0 truncate text-[11.5px] text-mute" title="TRF-2026-0917">TRF-2026-0917</div><div class="min-w-0 text-right">${moneyCell('Gs. 7.300.000')}<small class="ml-2 whitespace-nowrap tabular-nums text-mute">→ USD 1.000</small></div>`),
  v2Row('grid-cols-[minmax(20rem,1.6fr)_7rem_minmax(9rem,1.1fr)_minmax(9rem,1fr)_10rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Caja Chica Estudio → Banco Regional — Operativa">Caja Chica Estudio → Banco Regional — Operativa</b></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="15-sept-26 · 16:40">15-sept</span></div><div class="min-w-0 flex-1">${actorCompact('MG', 'María González')}</div><div class="min-w-0 text-[11.5px] text-mute">Sin referencia</div><div class="min-w-0 text-right">${moneyCell('Gs. 45.000.000')}</div>`),
  v2Row('grid-cols-[minmax(20rem,1.6fr)_7rem_minmax(9rem,1.1fr)_minmax(9rem,1fr)_10rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Banco Regional — Operativa → Caja Chica Estudio">Banco Regional — Operativa → Caja Chica Estudio</b></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="12-sept-26 · 09:15">12-sept</span></div><div class="min-w-0 flex-1">${actorCompact('FD', 'Fredd D.')}</div><div class="min-w-0 truncate text-[11.5px] text-mute" title="Reposición de caja">Reposición de caja</div><div class="min-w-0 text-right">${moneyCell('Gs. 12.000.000')}</div>`),
  v2Row('grid-cols-[minmax(20rem,1.6fr)_7rem_minmax(9rem,1.1fr)_minmax(9rem,1fr)_10rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Tarjeta corporativa USD → Banco Regional — Operativa">Tarjeta corporativa USD → Banco Regional — Operativa</b></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="10-sept-26 · 11:05">10-sept</span></div><div class="min-w-0 flex-1">${actorCompact('LG', 'Luis Giménez')}</div><div class="min-w-0 truncate text-[11.5px] text-mute" title="Venta de excedente">Venta de excedente</div><div class="min-w-0 text-right">${moneyCell('USD 2.500')}<small class="ml-2 whitespace-nowrap tabular-nums text-mute">→ Gs. 18.250.000</small></div>`),
 ]})}
</section>`,
};
...
<section class="grid gap-4">
 <section class="${v2Card}" aria-labelledby="finance-invoices-title">
  ...
  <div class="mb-4 flex flex-wrap items-end gap-3">
   ...
   <div class="relative min-w-0 w-full sm:w-64 [&>input]:!pl-9 [&>input]:!pr-9 [&>button]:h-11 [&>button]:w-11 md:[&>button]:h-7 md:[&>button]:w-7"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" class="shrink-0 pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-mute" aria-hidden="true"><path d="M11 19a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z"></path><path d="M21 21l-4.3-4.3"></path></svg><input class="w-full rounded-lg border border-ink-500 bg-ink-800 px-3.5 text-fore h-11 md:h-9 text-base md:text-sm outline-none transition focus:border-fono focus:ring-1 focus:ring-fono/40 placeholder:text-mute/60 pl-9 pr-9" placeholder="Número o cliente…" aria-label="Buscar factura o cliente" value=""></div>
   <p class="ml-auto whitespace-nowrap text-xs tabular-nums text-mute">5 de 24</p>
  </div>
  ${v2Grid({label: 'Cobros pendientes', template: 'grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_9rem]', columns: ['Factura', 'Estado', 'Vence', 'Pendiente', 'Total', 'Acciones'], minWidth: 'min-w-[60rem]', rows: [
   v2Row('grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="F-2026-0417 · Industrias del Sur Sociedad Anónima">F-2026-0417 · Industrias del Sur Sociedad Anónima</b></div><div class="min-w-0 flex-1">${chip('warn', 'Parcial')}</div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums font-semibold text-warn" title="18-sept-26">18-sept</span></div><div class="min-w-0 text-right">${moneyCell('Gs. 45.678.900')}</div><div class="min-w-0 text-right">${moneyCell('Gs. 90.000.000')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button whitespace-nowrap" type="button">+ Registrar cobro</button></div>`),
   v2Row('grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="F-2026-0412 · Grupo Comercial del Este SRL">F-2026-0412 · Grupo Comercial del Este SRL</b></div><div class="min-w-0 flex-1">${chip('bad', 'Vencida')}</div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums font-semibold text-warn" title="05-sept-26">05-sept</span></div><div class="min-w-0 text-right">${moneyCell('USD 12.500')}</div><div class="min-w-0 text-right">${moneyCell('USD 12.500')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button whitespace-nowrap" type="button">+ Registrar cobro</button></div>`),
   v2Row('grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="F-2026-0420 · Fundación Cultural Paraguaya">F-2026-0420 · Fundación Cultural Paraguaya</b></div><div class="min-w-0 flex-1">${chip('info', 'Emitida')}</div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="30-sept-26">30-sept</span></div><div class="min-w-0 text-right">${moneyCell('Gs. 8.500.000')}</div><div class="min-w-0 text-right">${moneyCell('Gs. 8.500.000')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button whitespace-nowrap" type="button">+ Registrar cobro</button></div>`),
   v2Row('grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="F-2026-0399 · Cliente sin factura emitida">F-2026-0399 · Cliente sin factura emitida</b></div><div class="min-w-0 flex-1">${chipMute('Borrador')}</div><div class="min-w-0 flex-1"><span class="text-[11px] text-mute">Sin fecha</span></div><div class="min-w-0 text-right"><span class="whitespace-nowrap text-[11px] text-mute">Sin saldo</span></div><div class="min-w-0 text-right">${moneyCell('Gs. 1.000.000')}</div><div class="flex min-w-0 items-center justify-end gap-1"></div>`),
  ]})}
 ...
 <section class="${v2Card}" aria-labelledby="finance-payments-title">
  ...
  ${v2Grid({label: 'Cobros registrados', template: 'grid-cols-[minmax(18rem,1.6fr)_6.5rem_minmax(9rem,1.1fr)_minmax(9rem,1.1fr)_minmax(8rem,1fr)_8.5rem_8rem]', columns: ['Cobro', 'Fecha', 'Cuenta', 'Recibió', 'Referencia', 'Monto', 'Acciones'], minWidth: 'min-w-[68rem]', rows: [
   v2Row('grid-cols-[minmax(18rem,1.6fr)_6.5rem_minmax(9rem,1.1fr)_minmax(9rem,1.1fr)_minmax(8rem,1fr)_8.5rem_8rem]', `<div class="flex min-w-0 items-center gap-2"><b class="truncate text-[13.5px] font-semibold text-fore" title="Industrias del Sur Sociedad Anónima · F-2026-0417">Industrias del Sur Sociedad Anónima · F-2026-0417</b></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="16-sept-26 · 15:20">16-sept</span></div><div class="min-w-0 truncate text-[11.5px] text-fore" title="Banco Regional — Operativa · Bancaria">Banco Regional — Operativa · Bancaria</div><div class="min-w-0 flex-1">${actorCompact('MG', 'María González')}</div><div class="min-w-0 truncate text-[11.5px] text-mute" title="Transferencia 8842">Transferencia 8842</div><div class="min-w-0 text-right">${moneyCell('Gs. 44.321.100')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button warn" type="button">${undoIcon(14)}Revertir cobro</button></div>`),
   v2Row('grid-cols-[minmax(18rem,1.6fr)_6.5rem_minmax(9rem,1.1fr)_minmax(9rem,1.1fr)_minmax(8rem,1fr)_8.5rem_8rem]', `<div class="flex min-w-0 items-center gap-2"><b class="truncate text-[13.5px] font-semibold text-fore" title="Grupo Comercial del Este SRL · F-2026-0390">Grupo Comercial del Este SRL · F-2026-0390</b><span class="inline-flex items-center gap-1 whitespace-nowrap rounded-md border border-warn/25 bg-warn/15 px-2 py-0.5 text-xs font-medium text-warn" title="Cobro imputado a la cuenta equivocada">Revertido</span></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="09-sept-26 · 11:40">09-sept</span></div><div class="min-w-0 truncate text-[11.5px] text-fore" title="Tarjeta corporativa USD · Digital">Tarjeta corporativa USD · Digital</div><div class="min-w-0 flex-1">${actorCompact('FD', 'Fredd D.')}</div><div class="min-w-0 text-[11.5px] text-mute">Sin referencia</div><div class="min-w-0 text-right">${moneyCell('USD 3.250,75')}</div><div class="flex min-w-0 items-center justify-end gap-1"></div>`),
  ]})}
  <div class="flex justify-end"><button class="secondary" type="button">Ver todos los cobros</button></div>
 </section>
</section>`,
};
...
<section class="grid gap-4" aria-label="Cobranza y mora">
 <header class="mb-4 flex flex-wrap items-start justify-between gap-x-4 gap-y-3">
  <div class="min-w-0 flex-1">
   <p class="mb-1.5 font-mono text-[10px] uppercase tracking-[.14em] text-mute">Finanzas</p>
   <h1 class="text-[22px] font-bold leading-tight tracking-tight text-fore md:text-2xl">Estado de pagos</h1>
   <p class="mt-1.5 text-[13px] leading-[1.5] text-mute">Saldo pendiente por antigüedad y días en calle por moneda.</p>
  </div>
  <div class="flex min-w-0 flex-wrap items-center gap-2 max-md:w-full max-md:justify-start"><span class="whitespace-nowrap text-xs tabular-nums text-mute">Actualizado 22-sept-26 · 09:30</span></div>
 </header>
...
<div role="row" class="${FORECAST_ROW} ${PLANNED_COLS}">
 ${cell(`<b class="font-semibold text-fore">${category}</b>${kind ? `<span class="ml-2 text-[10px] font-bold uppercase tracking-wider text-mute">${kind}</span>` : ''}<small class="ml-2 text-xs text-mute">${note}</small>`, 'truncate text-sm').replace('<span', `<span title="${category}${note ? ` · ${note}` : ''}"`)}
 ${cell(`<span class="text-sm text-fore">${cadence}</span>`)}
 ${cell(`<strong class="whitespace-nowrap text-sm font-semibold tabular-nums text-fore">${amount}</strong>`, 'text-right')}
 ${cell(`<span class="[&>button]:h-11 [&>button]:w-11 md:[&>button]:h-7 md:[&>button]:w-7">${iconAction('<path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7V4h6v3"/>', `Quitar gasto planificado: ${category}`, 'bad')}</span>`, 'flex justify-end')}
</div>`;
const realExpenseRow = ({category, kind, date, account, reference, createdBy, amount}) => `
<div role="row" class="${FORECAST_ROW} ${EXPENSE_COLS}">
 ${cell(`<b class="font-semibold text-fore">${category}${kind ? ` · ${kind}` : ''}</b><small class="ml-2 text-xs text-mute">${date} · ${account}${reference ? ` · ${reference}` : ''}${createdBy ? ` · registró ${createdBy}` : ''}</small>`, 'truncate text-sm').replace('<span', `<span title="${category} · ${date} · ${account}${reference ? ` · ${reference}` : ''}"`)}
 ${cell(`<strong class="whitespace-nowrap text-sm font-semibold tabular-nums text-fore">${amount}</strong>`, 'text-right')}
 ${cell(`<span class="[&>button]:h-11 [&>button]:w-11 md:[&>button]:h-7 md:[&>button]:w-7">${iconAction('<path d="M3 7v6h6"/><path d="M3 13a9 9 0 1 0 3-7.7L3 8"/>', `Revertir gasto real: ${category}`, 'bad')}</span>`, 'flex justify-end')}
</div>`;
...
<section class="grid gap-4" aria-label="Previsión financiera">
 <header class="mb-4 flex flex-wrap items-start justify-between gap-x-4 gap-y-3">
  <div class="min-w-0 flex-1">
   <p class="mb-1.5 font-mono text-[10px] uppercase tracking-[.14em] text-mute">Finanzas</p>
   <h1 class="text-[22px] font-bold leading-tight tracking-tight text-fore md:text-2xl">Previsión financiera</h1>
...
<section class="grid gap-4" aria-label="Reportes de la agencia">
 <header class="mb-4 flex flex-wrap items-start justify-between gap-x-4 gap-y-3">
  <div class="min-w-0 flex-1">
   <p class="mb-1.5 font-mono text-[10px] uppercase tracking-[.14em] text-mute">Informes</p>
   <h1 class="text-[22px] font-bold leading-tight tracking-tight text-fore md:text-2xl">Evolución mensual</h1>
   <p class="mt-1.5 text-[13px] leading-[1.5] text-mute">Importes registrados, no utilidad ni rentabilidad. Las monedas se consultan por separado.</p>
  </div>
  <div class="flex min-w-0 flex-wrap items-center gap-2 max-md:w-full max-md:justify-start"></div>
 </header>
...
<section class="${v2Card}" aria-labelledby="commissions-list-title">
 <div class="flex flex-wrap items-center justify-between gap-2">
  <div class="min-w-0 flex-1"><h3 id="commissions-list-title" class="text-[17px] font-semibold tracking-tight text-fore">Comisiones y referidos</h3><p class="mt-1 text-xs text-mute">Los porcentajes se calculan al registrar la comisión; los cobros posteriores no modifican acuerdos ya registrados.</p></div>
...
<section class="grid gap-4">
 <section class="${v2Card}" aria-labelledby="commissions-discounts-title">
  ...
  ${v2Grid({label: 'Descuentos por referido', template: 'grid-cols-[minmax(16rem,1.4fr)_minmax(22rem,1.5fr)_7rem_9rem_9rem]', columns: ['Referido', 'Factura y cliente', 'Monto', 'Estado', 'Acciones'], minWidth: 'min-w-[64rem]', rows: [
   v2Row('grid-cols-[minmax(16rem,1.4fr)_minmax(22rem,1.5fr)_7rem_9rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore">Estudio Contable Ramírez</b><small class="block truncate text-[11px] text-mute">Alta 14-sept</small></div><div class="min-w-0 truncate text-[11.5px] text-mute" title="F-2026-0417 · Industrias del Sur · recomendación directa"><span class="text-fore">F-2026-0417</span><span> · Industrias del Sur · recomendación directa</span></div><div class="min-w-0 text-right">${moneyCell('Gs. 2.000.000')}</div><div class="min-w-0 flex-1">${chip('ok', 'Aplicado')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button warn" type="button">${undoIcon(14)}Revertir</button></div>`),
   v2Row('grid-cols-[minmax(16rem,1.4fr)_minmax(22rem,1.5fr)_7rem_9rem_9rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore">María González</b><small class="block truncate text-[11px] text-mute">Sin fecha de alta</small></div><div class="min-w-0 truncate text-[11.5px] text-mute" title="F-2026-0390 · Grupo Comercial · acuerdo comercial anual"><span class="text-fore">F-2026-0390</span><span> · Grupo Comercial · acuerdo comercial anual</span></div><div class="min-w-0 text-right">${moneyCell('USD 250')}</div><div class="min-w-0 flex-1">${chipMute('Revertido')}</div><div class="flex min-w-0 items-center justify-end gap-1"></div>`),
  ]})}
 </section>
 <section class="${v2Card}" aria-labelledby="commissions-payouts-title">
  ...
  ${v2Grid({label: 'Pagos registrados', template: 'grid-cols-[minmax(24rem,1.6fr)_6.5rem_minmax(10rem,1.2fr)_minmax(10rem,1.1fr)_8.5rem]', columns: ['Egreso', 'Fecha', 'Cuenta', 'Registró', 'Monto'], minWidth: 'min-w-[70rem]', rows: [
   v2Row('grid-cols-[minmax(24rem,1.6fr)_6.5rem_minmax(10rem,1.2fr)_minmax(10rem,1.1fr)_8.5rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Ana López Fernández de la Cruz">Ana López Fernández de la Cruz</b><small class="block truncate text-[11px] text-mute" title="Comisión F-2026-0390 · agosto 2026">Comisión F-2026-0390 · agosto 2026</small></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="20-sept-26">20-sept</span></div><div class="min-w-0 text-[11.5px] text-fore">Banco Regional — Operativa</div><div class="min-w-0 text-[11.5px] text-mute">finanzas@estudio.com.py</div><div class="min-w-0 text-right">${moneyCell('Gs. 45.678.900')}</div>`),
   v2Row('grid-cols-[minmax(24rem,1.6fr)_6.5rem_minmax(10rem,1.2fr)_minmax(10rem,1.1fr)_8.5rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="Luis Giménez">Luis Giménez</b><small class="block truncate text-[11px] text-mute">Sin referencia</small></div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="18-sept-26">18-sept</span></div><div class="min-w-0 text-[11.5px] text-fore">Tarjeta corporativa USD</div><div class="min-w-0 text-[11.5px] text-mute">Sin registrar</div><div class="min-w-0 text-right">${moneyCell('USD 1.250,75')}</div>`),
  ]})}
 </section>
</section>`,
};
