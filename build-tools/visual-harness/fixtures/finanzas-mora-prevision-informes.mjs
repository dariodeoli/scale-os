...
<section class="grid gap-4">
 <section class="${v2Card}" aria-labelledby="finance-invoices-title">
  ...
  ${v2Grid({label: 'Cobros pendientes', template: 'grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_8rem]', columns: ['Factura', 'Estado', 'Vence', 'Pendiente', 'Total', 'Acciones'], minWidth: 'min-w-[60rem]', rows: [
   ...
   v2Row('grid-cols-[minmax(18rem,1.6fr)_7rem_6.5rem_8.5rem_8.5rem_8rem]', `<div class="min-w-0 flex-1"><b class="block truncate text-[13.5px] font-semibold text-fore" title="F-2026-0420 · Fundación Cultural Paraguaya">F-2026-0420 · Fundación Cultural Paraguaya</b></div><div class="min-w-0 flex-1">${chip('info', 'Emitida')}</div><div class="min-w-0 flex-1"><span class="whitespace-nowrap tabular-nums text-fore" title="30-sept-26">30-sept</span></div><div class="min-w-0 text-right">${moneyCell('Gs. 8.500.000')}</div><div class="min-w-0 text-right">${moneyCell('Gs. 8.500.000')}</div><div class="flex min-w-0 items-center justify-end gap-1"><button class="text-button" type="button">+ Registrar cobro</button></div>`),
