...
<article class="work-card identity-card identity-${order.color}">
 <div class="card-top">
  <button class="text-button order-open" aria-label="Abrir ${order.title}">${order.title}</button>
...
<section class="column">
 ...
 ${orders.map(workCard).join('')}
</section>`;

const productionColumns = [
  {
    status: {id: 'blocked', label: 'Bloqueado', tone: 'red'},
    orders: [
      {
        title: 'Reel de lanzamiento para la nueva línea de productos — corte final con subtítulos, corrección de color y mezcla',
        color: 'violet',
        client: {name: 'Cooperativa Multiactiva de Servicios Múltiples Limitada', initials: 'CM', color: 'teal'},
        project: 'Campaña Aniversario 2026 · Temporada de verano · Spots para televisión abierta y redes',
        urgency: '5 · Crítica',
        drive: '#drive',
        description: 'Falta la aprobación del cliente sobre la música y el cierre con el logo animado de cierre.',
        due: {title: '28 ago. 2026 09:30', text: 'Entrega 28 ago. 2026 · 09:30 h · venció hace 23 días', overdue: true},
        people: [{initials: 'MR', name: 'María Renée Ayala Benítez', primary: true}, {initials: 'JC', name: 'Juan Carlos Villalba'}],
        checklist: {done: 3, total: 8},
      },
    ],
  },
  {
    status: {id: 'to_record', label: 'Por grabar', tone: 'yellow'},
    orders: [
      {
        title: 'Spots de 15 s y 30 s para radio, televisión abierta y redes sociales',
        color: 'blue',
        client: {name: 'Estudio de Comunicación y Producción Audiovisual del Paraguay Sociedad Anónima', initials: 'EC', color: 'violet'},
        project: 'Documental institucional del Bicentenario — Investigación, rodaje y postproducción completa',
        urgency: '2 · Moderada',
        drive: null,
        description: 'Pendiente confirmar locación y permisos de filmación en el centro histórico.',
        due: {title: '23 sept. 2026 07:00', text: 'Entrega 23 sept. 2026 · 07:00 h · faltan 3 días', overdue: false},
        people: [{initials: 'LP', name: 'Lucía Paredes', primary: true}],
        checklist: null,
      },
    ],
  },
  {
    status: {id: 'recorded', label: 'Grabado', tone: 'teal'},
    orders: [
      {
        title: 'Cobertura multicámara del evento de aniversario con entrevistas y piezas verticales para Instagram y TikTok',
        color: 'green',
        client: {name: 'Fundación Niñez y Comunidad', initials: 'FN', color: 'rose'},
        project: 'Cobertura de eventos corporativos 2026',
        urgency: '3 · Media',
        drive: '#drive',
        description: null,
        due: null,
  ...
  {
    status: {id: 'editing', label: 'Editando', tone: 'purple'},
    orders: [
      {
        title: 'Edición de cápsulas testimoniales para la campaña de responsabilidad social',
        color: 'gold',
        client: {name: 'Ministerio de Educación y Ciencias', initials: 'ME', color: 'blue'},
        project: 'Memoria audiovisual 2026',
        urgency: 'Sin definir',
        drive: '#drive',
        description: null,
        due: {title: '30 sept. 2026 18:30', text: 'Entrega 30 sept. 2026 · 18:30 h · faltan 10 días', overdue: false},
        people: [{initials: 'RS', name: 'Rodrigo Sosa Giménez', primary: true}],
        checklist: {done: 1, total: 4},
      },
    ],
  },
  {status: {id: 'review', label: 'Revisión', tone: 'blue'}, orders: []},
  {status: {id: 'approved', label: 'Aprobado', tone: 'green'}, orders: []},
...
<div class="production-view-menu production-toolbar">
 ...
 <div class="production-filters">
  ${selectCustom('Filtrar por cliente', 'Cooperativa Multiactiva de Servicios Múltiples Limitada', 'fc')}
  <button type="button" class="text-button">${svg(14, ICON.slidersHorizontal)}Filtros · 2</button>
  <p class="production-filter-summary" role="status" aria-live="polite">62 de 74 órdenes</p>
  <button class="text-button">${svg(14, ICON.rotateCcw)}Restablecer filtros</button>
 </div>
 <button class="text-button production-project-link">Ver proyectos${svg(14, ICON.arrowUpRight)}</button>
</div>
<p class="form-note">Filtros guardados del tablero · Todos los estados. La semana va de lunes a domingo según la hora local de tu dispositivo.</p>`;

const productionPanel = `
<section class="panel production-panel production-focus" id="produccion">
 <div class="kanban" tabindex="0" role="region" aria-label="Tablero de Producción, desplazable horizontalmente">
  ${productionColumns.map((column) => kanbanColumn(column.status, column.orders)).join('')}
 </div>
 <p class="board-note">Arrastrá una orden de una columna a otra para actualizar su estado.</p>
</section>`;
...
<article class="project-entry" id="project-${project.id}" tabindex="-1">
 ...
 <div class="project-entry-meta"><span class="client-status" data-status="${project.status.key}">${project.status.label}</span><small class="urgency-badge">Urgencia: ${project.urgency}</small></div>
 <dl class="project-entry-facts">
  <div><dt>Inicio</dt><dd class="list-date">${project.start || 'Sin fecha'}</dd></div>
...
const projectHeadRow = '<div class="project-entry-head" aria-hidden="true"><span>Proyecto</span><span>Estado</span><span>Fechas y piezas</span><span>Responsables</span><span>Acciones</span></div>';

const projects = [
  {
    id: '01J7Q2',
    name: 'Campaña Aniversario 2026 · Temporada de verano · Spots para televisión abierta, radio y redes sociales',
    client: {name: 'Cooperativa Multiactiva de Servicios Múltiples Limitada', initials: 'CM', color: 'teal'},
    ...
    dueTone: 'warn',
    pieces: 148,
    people: [{initials: 'MR', name: 'María Renée Ayala Benítez', primary: true}, {initials: 'JC', name: 'Juan Carlos Villalba'}, {initials: 'LP', name: 'Lucía Paredes'}],
    drive: '#drive',
    selectable: true,
    manageable: true,
  },
  ...
  {
    id: '01J7Q4',
    name: 'Cobertura de eventos corporativos 2026',
    client: {name: 'Fundación Niñez y Comunidad', initials: 'FN', color: 'rose'},
    status: {key: 'completed', label: 'Completado'},
    urgency: 'Sin definir',
    start: '02-feb',
    due: '14-ago',
    dueTone: '',
  ...
  {
    id: '01J7Q5',
    name: 'Memoria audiovisual 2026 · Registro de archivo histórico y digitalización de cintas',
    client: {name: 'Ministerio de Educación y Ciencias', initials: 'ME', color: 'blue'},
    ...
    dueTone: '',
    pieces: 12,
    people: [{initials: 'LP', name: 'Lucía Paredes', primary: true}, {initials: 'MR', name: 'María Renée Ayala Benítez'}],
    drive: '#drive',
    selectable: true,
    manageable: true,
  },
...
export default [
  ...
  },
  {
    ...
    section: 'Producción',
    surface: 'Tablero por etapas (scroll horizontal)',
    kind: 'workspace',
    body: `${productionToolbar}${productionPanel}`,
  },
  {
    id: 'proyectos-lista',
    section: 'Proyectos',
    ...
    lists: [
      {
        ...
        head: '.project-entry-head',
        row: '.project-list .project-entry',
  ...
  },
  {
    ...
    kind: 'workspace',
    grids: [{container: '.project-grid', card: '.project-entry', label: 'Proyectos · cuadrícula', minHeight: 200}],
    body: `<div class="project-grid">${projects.map(projectEntry).join('')}</div>`,
  },
  {
