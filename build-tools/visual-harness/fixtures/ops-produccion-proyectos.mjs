...
<article id="project-${project.id}" tabindex="-1" class="project-entry group/project grid min-h-[48px] min-w-0 grid-cols-[var(--project-cols)] items-center gap-x-2 rounded-xl border border-ink-600 px-3 py-1">
 <div class="flex min-w-0 items-center gap-2">${project.selectable ? `<label class="select-check" title="Seleccionar proyecto"><input type="checkbox" class="h-4 w-4 p-0 accent-fono" aria-label="Seleccionar ${project.name}"></label>` : ''}<div class="flex min-w-0 items-center gap-2"><h3 class="truncate text-sm font-semibold text-fore" title="${project.name}">${project.name}</h3><span class="hidden min-w-0 truncate text-[11.5px] text-mute">· ${project.client.name}</span></div></div>
 <div class="flex min-w-0 flex-nowrap items-center gap-1">${chip(project.status.label, project.status.tone)}<span class="hidden">${urgencyBadge(project.urgency)}</span></div>
 <dl class="flex min-w-0 flex-nowrap items-center gap-x-3 overflow-hidden whitespace-nowrap text-[11.5px]" title="Inicio ${project.start || 'sin fecha'} · Entrega ${project.due || 'sin fecha'} · ${project.pieces} piezas"><div class="flex items-center gap-1.5"><dt class="hidden text-mute">Inicio</dt><dd class="list-date whitespace-nowrap">${project.start || 'Sin fecha'}</dd></div><div class="flex items-center gap-1.5"><dt class="hidden text-mute">Entrega</dt><dd class="list-date whitespace-nowrap"${project.dueTone ? ` data-tone="${project.dueTone}"` : ''}>${project.due || 'Sin fecha'}</dd></div><div class="flex items-center gap-1.5"><dt class="hidden text-mute">Piezas</dt><dd class="tabular-nums">${project.pieces}</dd><span class="text-mute"> piezas</span></div>${project.levels ? `<div class="flex items-center gap-1.5" title="Niveles de aprobación interna: ${project.levels}"><dt class="text-mute">Aprobación</dt><dd class="tabular-nums">${project.levels}</dd></div>` : ''}</dl>
 <span class="min-w-0 truncate text-[11.5px] text-mute" title="${(project.people||[]).map(p=>p.name).join(', ')}">${(project.people||[]).map(p=>p.name).join(', ') || (project.inherited ? 'Responsables no disponibles' : 'Sin responsables')}</span>
 <p class="hidden text-[10.5px] text-mute">Actualizado ${project.updated}</p>
 <div class="mt-0 flex min-w-0 flex-nowrap items-center gap-2 overflow-x-auto border-t-0 pt-0">${project.links ? `<a class="whitespace-nowrap text-[11.5px] font-semibold text-fono-light" href="#drive">Abrir Drive${project.links > 1 ? ` (${project.links})` : ''} ↗</a>` : '<small class="whitespace-nowrap text-[11.5px] text-mute">Sin Drive</small>'}${iconAction({icon: 'eye', label: `Ver detalle del proyecto: ${project.name}`, tone: 'fono'})}<button type="button" class="text-button">Comentarios</button>${project.manageable ? '<button type="button" class="text-button">Archivar</button>' : ''}${project.manageable ? iconAction({icon: 'pencil', label: `Editar proyecto: ${project.name}`}) : ''}</div>
</article>`;
