...
<section class="flex min-w-0 w-[calc((100%-(var(--board-cols)-1)*0.75rem)/var(--board-cols))] shrink-0 snap-start flex-col gap-2 rounded-xl border border-ink-600 bg-ink-800/60 p-2.5" data-column="${status.id}">
...
<div class="flex flex-wrap items-center justify-between gap-2">
 ...
 <div class="flex items-center gap-1" role="group" aria-label="Recorrido del tablero" data-board-window>
  ...
  <span class="whitespace-nowrap text-[11px] tabular-nums text-mute" title="Se ven las etapas 1 a 4 de 7. Usá las flechas o deslizá el tablero.">Etapas 1–4 de 7</span>
...
export default [
  {
    ...
    lists: [],
    grids: [],
    body: `<div class="grid min-w-0 gap-4">${productionToolbar}<section class="grid min-w-0 gap-2" id="produccion" aria-label="Tablero de Producción">${boardWindow}<div class="silent-scroll flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 [--board-cols:1] sm:[--board-cols:2] lg:[--board-cols:4]" tabindex="0" role="region" aria-label="Tablero de Producción, desplazable por bloques de etapas">${productionColumns.map(column => kanbanColumn(column.status, column.orders, column.status.id === 'to_record' ? {count: 430, hasMore: true} : {})).join('')}</div></section></div>`,
  },
  {
    id: 'produccion-vacio',
    section: 'Producción',
